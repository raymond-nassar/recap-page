...
test('five reports bind the complete library, four peers, and central approvals', async () => {
  ...
  for (const id of CBRO_SELECTED_IDS) {
    ...
    assert.equal(report.comparisonCount, 135);
    assert.match(report.libraryDigest, /^[0-9a-f]{64}$/);
...
test('continuation reports bind 671 comparisons and one central subset approval', async () => {
  const library = await loadLibrarySnapshot();
  const reviewedLibraryDigest = libraryDigestExcludingOrders(
    library,
    [
...
test('historical continuation authoring retains all chronological cards and 32 batch-one payload rows', async () => {
  ...
  assert.deepEqual(
    ...
    [
      ...CBRO_CONTINUATION_AUTHOR_IDS.slice(1),
      ...CBRO_BATCH_TWO_AUTHOR_IDS,
...
test('batch two authoring ships five chronological cards and 35 exact payload rows', async () => {
  ...
  const manifest = await readJson(path.join(dataDir, 'curated-lists.json'));
  const catalog = await readJson(path.join(dataDir, 'catalog.json'));
  assert.equal(manifest.lists.length, 149);
  assert.equal(catalog.lists.length, 149);
...
test('batch five authoring ships four chronological cards and 71 exact payload rows', async () => {
  ...
  assert.deepEqual(
    ...
    [...CBRO_BATCH_FIVE_AUTHOR_IDS.slice(1), 'x-tinction-agenda'],
  );
  assert.equal(manifest.lists.length, 149);
  assert.equal(catalog.lists.length, 149);
...
test('batch six reports bind 496 all-none comparisons and reject stale evidence', async () => {
  const expectedComparisonDigests = new Map([
    ['x-tinction-agenda', '0b8dbdcfec446c987517ad14e1d49aa7950d0f6e532aafd641e4a649091dcf4a'],
    ['operation-galactic-storm', '46d96f8377e1bd384272464e2c0297bb9cb3e43e3208a35363dc71352c788805'],
    ['dead-mans-hand', 'df606ffb367871485a43402be1d9333ca8e73dd1dde81816ea6459819ad8c9b5'],
    ['rise-of-the-midnight-sons', '36f2dbea342c55bdc90113d8413e9e156fcbe5c3eb3e7d81a5025c453c9d95ef'],
  ]);
  const library = await loadLibrarySnapshot();
  const reviewedLibraryDigest = libraryDigestExcludingOrders(
    library,
...
test('batch six authoring ships four chronological cards and 46 exact payload rows', async () => {
  ...
  assert.deepEqual(
    ...
    [...CBRO_BATCH_SIX_AUTHOR_IDS.slice(1), 'x-cutioners-song'],
  );
  assert.equal(manifest.lists.length, 149);
  assert.equal(catalog.lists.length, 149);
...
test('batch eight packets mappings reports and product outputs preserve 45 exact rows', async () => {
  ...
  const expectedDigests = new Map([
    ['time-and-time-again', {
      ...
      report: 'f2237a3247fb130c110386b37cd3fff4962399ea05b771ed91495e93ac9702d6',
      approval: 'bad254b31c693f39740b9e52835f33cca94cb6cca6806a72eff16fcfdf9d8113',
    }],
    ['phalanx-covenant', {
      ...
      report: '47df0171f34f18f29feba55b1c6f1d6dbf80013e93ec1adb71af1ff608abf405',
      approval: '16c85387fb00064d4af4198ca5cc84281296d3792c3267039806d6f53c73c92a',
    }],
    ['operation-zero-tolerance', {
      ...
      report: '7945fc71823affd1c0b690b00e39c9c89159bb34d423f708ab91555ebbe3c69f',
      approval: '3c83b653de622394c8425d8f8ab797b77be9d6c4bc9e193193b4c752bcaedcbf',
    }],
    ['spider-man-identity-crisis', {
      ...
      report: '3fba86d97cb15a3f82a7d3f494eb91911627fd4c7273e7b2cdf12c3cc7261cb9',
      approval: 'b454c0b62ca814e0de4ef46d13c4816c44b21e619c94c5417c0925781b4950c3',
    }],
  ]);
...
test('batch nine packets mappings and reports preserve 14 exact all-none rows', async () => {
  ...
  const expectedDigests = new Map([
    ['hunt-for-xavier', {
      ...
      report: '18feaa6e71742d9a6ae4b66cf14f60ece29fec7e590171b36020fac50e08fb2e',
      approval: '0753ddb3204e96bc28a96367df4b54277881a539859091e58ff9c6c2df64ce54',
    }],
    ['magneto-war', {
      ...
      report: 'b2e23620ad2bc7b6b0552f44701006fc989c12f51151a4b068932971e6de02b2',
      approval: 'e625c71940735167c8b9c777a244a26403388d42d9b246ed427a19ed08cb02ca',
    }],
  ]);
  const mappingById = new Map();
...
test('batch seven packets mappings reports and product outputs preserve 23 exact rows', async () => {
  ...
  const expectedDigests = new Map([
    ['x-cutioners-song', {
      ...
      report: '23c80c96f8581dd1d1a6851ebbfc52a4d658713a74ddfd9e4c545fa678dd26c0',
      approval: 'c8e85f96abb00009448891e1116d97c4a4237064e757b565c3e7896f44ba61bc',
    }],
    ['mys-tech-wars', {
      ...
      report: '7fb6188b426e4d9835bdd24c5f0dac3ed42b15b0f6a8ac2e300e1038c982f67b',
      approval: '11ad8af2c74284a3e8048653b7465717594472f705f10ed8c6e07272d1ff2d46',
    }],
    ['fatal-attractions', {
      ...
      report: '268afea40d927210ba6cbe98e3ed432e322c35272a9a60f737fbbf8482d9c951',
      approval: '9a136aa8e3071298b182773b3767d5ae437802d7541db5b5e71029d2790c5a14',
    }],
  ]);
  const mappingById = new Map();
