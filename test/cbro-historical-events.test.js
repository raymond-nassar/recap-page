...
import {
  ...
  CBRO_MARVEL_2099_EXCLUSION_SHA256,
  CBRO_MARVEL_2099_SELECTED_IDS,
...
function libraryDigestForReportScope(library, mapping, peerMappings, report) {
  const comparisonOrderIds = new Set(report.comparisons.map((comparison) => comparison.orderId));
  return libraryDigestExcludingOrders(
    library,
    [
      mapping.id,
      ...peerMappings.map((peer) => peer.id),
      ...library.manifest.lists
        .map((entry) => entry.id)
        .filter((id) => !comparisonOrderIds.has(id)),
    ],
  );
}
...
test('CBRO shares repeated-source counts, provenance, and approval derivation', () => {
  ...
  assert.throws(
    ...
    /historical-event mapping source position evidence differs from its frozen packet/i,
  );
});
...
test('five reports bind the complete library, four peers, and central approvals', async () => {
  ...
  for (const id of CBRO_SELECTED_IDS) {
    ...
    assert.deepEqual(
      ...
      id === 'bloodties'
        ? [{ orderId: 'black-widow-reading-order', relationship: 'candidate-subset', sharedCount: 5 }]
        : [],
    );
...
test('continuation reports bind 675 comparisons and one central subset approval', async () => {
  const library = await loadLibrarySnapshot();
  const reviewedLibraryDigest = libraryDigestExcludingOrders(
    library,
    [
  ...
  assert.deepEqual(nonNone, [
    ...
    },
    {
      ...
      sharedCount: 8,
      sharedIds: ['6970', '20392', '6971', '20403', '6972', '20263', '6973', '20274'],
      relationship: 'candidate-subset',
    },
    {
      ...
      sharedCount: 12,
      sharedIds: ['9610', '8014', '8015', '8016', '8017', '8018', '23322', '8019', '8020', '8021', '6981', '8022'],
      relationship: 'candidate-subset',
    },
  ]);
...
test('batch three authoring ships three chronological cards and 48 exact payload rows', async () => {
  ...
  const manifest = await readJson(path.join(dataDir, 'curated-lists.json'));
  const catalog = await readJson(path.join(dataDir, 'catalog.json'));
  assert.equal(manifest.lists.length, 174);
...
test('batch five reports bind 540 all-none comparisons and reject stale evidence', async () => {
  const library = await loadLibrarySnapshot();
  const reviewedLibraryDigest = libraryDigestExcludingOrders(
    library,
    [
  ...
  for (const id of CBRO_BATCH_SIX_SELECTED_IDS) {
    ...
    assert.deepEqual(
      ...
      id === 'operation-galactic-storm'
        ? [{ orderId: 'black-widow-reading-order', relationship: 'partial', sharedCount: 17 }]
        : [],
    );
    assert.doesNotThrow(() => validateCbroReviewIdentity(mapping));
    assert.doesNotThrow(() => assertApprovedRelationshipReview({
...
test('batch six authoring ships four chronological cards and 46 exact payload rows', async () => {
  ...
  assert.match(provenance, /Thirty-nine lists in this repository now come from that site/);
  const maintaining = await readFile(path.join(root, 'docs', 'MAINTAINING.md'), 'utf8');
  assert.match(maintaining, /currently ships 39 guides/);
...
test('batch seven authority preserves seven outcomes and four complete blocker records', async () => {
  ...
  const preBatchEightSelectedIds = CBRO_ALL_SELECTED_IDS.filter(
    (id) => !CBRO_BATCH_EIGHT_SELECTED_IDS.includes(id)
      && !CBRO_BATCH_NINE_SELECTED_IDS.includes(id)
      && !CBRO_MARVEL_2099_SELECTED_IDS.includes(id),
  );
  ...
  assert.equal(next.centralDisposition, 'selected');
  assert.equal(next.deliveryStatus, 'shipped');
...
test('batch eight packets mappings reports and product outputs preserve 45 exact rows', async () => {
  ...
  const expectedDigests = new Map([
    ['time-and-time-again', {
      ...
      report: 'e04745ba82e71755ec4ea1007f951c065757bc01b53e3d0660c3f45109bccd6b',
      approval: '10ac7195fbf01d20a5e91b70d85ed80a773743883921f34865647df3433b6375',
    }],
    ['phalanx-covenant', {
      ...
      report: '05c386b1773dbee4ca2957342da82b4c6ae39d093c413e906011a0cca4dcf17e',
      approval: 'd85957abd31bcb9a32f318adb23d51d3289f45f2ea376faa3cf08c2245263313',
    }],
    ['operation-zero-tolerance', {
      ...
      report: '83fabe5a43035bd065f6732ec1257c82816f3643bd6672f25d8d2e80b7f5cf89',
      approval: 'dc9fef58b9cadcedbcd7329bc198750c4e2a7a8f279970cfd6f1cfdc09a90d95',
    }],
    ['spider-man-identity-crisis', {
      ...
      report: '514f276488acd9846c30554939260ba5c3714a0003c5b78ba935d835459ae5b7',
      approval: '01e60cef23eda2781afe6d4c97e580bda0ea0904f703780c6a4c36c0a2d2c306',
    }],
  ]);
...
test('Marvel 2099 publishes 172 exact issues while conserving all 271 source positions', async () => {
  const inventory = await readJson(path.join(root, 'scripts', 'data', 'cbro-historical-inventory.json'));
  const inventoryRecord = inventory.find((record) => record.id === 'marvel-2099');
  const [packet, mapping, report, manifest, catalog, payload, markdown] = await Promise.all([
    readJson(path.join(packetsDir, 'marvel-2099.json')),
    readJson(path.join(mappingsDir, 'marvel-2099.json')),
    readJson(path.join(overlapsDir, 'marvel-2099.json')),
    readJson(path.join(dataDir, 'curated-lists.json')),
    readJson(path.join(dataDir, 'catalog.json')),
    readJson(path.join(dataDir, 'marvel_2099.json')),
    readFile(path.join(dataDir, 'orders', 'marvel-2099.md'), 'utf8'),
  ]);
  assert.doesNotThrow(() => validateCbroHistoricalInventory(inventory));
  assert.doesNotThrow(() => validateCbroPacket(packet, {
    inventoryRecord,
  ...
  assert.doesNotThrow(() => validateMappingDigest(mapping));
  assert.doesNotThrow(() => validateCbroReviewIdentity(mapping));
  assert.deepEqual(CBRO_MARVEL_2099_SELECTED_IDS, ['marvel-2099']);
  assert.equal(inventoryRecord.sourceRowCount, 271);
  assert.equal(inventoryRecord.centralDisposition, 'selected');
  assert.equal(inventoryRecord.deliveryStatus, 'shipped');
  assert.equal(inventoryRecord.universeScope, 'alternate');
  assert.deepEqual(inventoryRecord.catalogIds, ['marvel-2099']);
  assert.deepEqual(inventoryRecord.overlapIds, ['fantastic-four-reading-order']);
  assert.equal(packet.rows.length, 172);
  ...
  assert.deepEqual(
    ...
    Array.from({ length: 271 }, (_value, index) => index + 1),
  );
  assert.equal(
  ...
  assert.ok(packet.excludedSourceRows.every((row) => (
    row.exclusionKind === 'metadata-unavailable'
      && row.lookupResult
      && row.evidenceDigest
  )));
  assert.equal(mapping.rows.length, 172);
  assert.equal(new Set(mapping.rows.map((row) => String(row.selectedIssueId))).size, 172);
  assert.equal(report.comparisonCount, 170);
  assert.deepEqual(
  ...
  assert.equal(manifest.lists.find((entry) => entry.id === 'marvel-2099').expect, 172);
  assert.equal(catalog.lists.find((entry) => entry.id === 'marvel-2099').count, 172);
  assert.equal(payload.items.length, 172);
  assert.equal(parseChecklist(markdown).entries.length, 172);
  assert.match(markdown, /271 issue positions/i);
  assert.match(markdown, /172 distinct issues/i);
  assert.match(markdown, /1 repeated occurrence/i);
  assert.match(markdown, /omits 98 positions/i);
  assert.doesNotMatch(markdown, /nonessential tie-in/i);

  const missingGap = structuredClone(packet);
  missingGap.excludedSourceRows.pop();
  missingGap.packetDigest = packetDigestFor(missingGap);
  assert.throws(() => validateCbroPacket(missingGap, { inventoryRecord }), /sourceOccurrenceCount/i);

  const changedLookup = structuredClone(packet);
  changedLookup.excludedSourceRows[0].lookupResult = 'invented';
  changedLookup.packetDigest = packetDigestFor(changedLookup);
  assert.throws(() => validateCbroPacket(changedLookup, { inventoryRecord }), /evidence digest is stale/i);
});
...
test('batch nine packets mappings and reports preserve 14 exact all-none rows', async () => {
  ...
  const expectedDigests = new Map([
    ['hunt-for-xavier', {
      ...
      report: '699b54fa16d3cb1b32dfae4463891f4ab378e5802f2b32ca98d1a969462af4e1',
      approval: '06b8f138cf68a647de634b5ea3988ef988353cbb39614c7eb07d831430da1722',
    }],
    ['magneto-war', {
      ...
      report: '3092d447cf860917ffc148d7086ff12492080baacafeaa10b44832447db6080b',
      approval: 'ed9c755a8278de1eafe318d40b79d44e00b593e985eda49867fa0def08d414e0',
    }],
  ]);
  const mappingById = new Map();
...
test('batch seven packets mappings reports and product outputs preserve 23 exact rows', async () => {
  ...
  const expectedDigests = new Map([
    ['x-cutioners-song', {
      ...
      report: '1464dacb2deddac3650a2fbdbee831e45a485a1ec64a5348aa48f42e57c21f64',
      approval: '7fd73fad2ce6984b5c8e249e24a5e5f55de9aee232bab52c1ea01558852ac959',
    }],
    ['mys-tech-wars', {
      ...
      report: 'ad8e7f0fb80e7bf77a9bd2412166bb0f03f46d44884a3c8f5b83ae034a1ae3b8',
      approval: 'afa2e5eb92018a3728cc74cbf3e90ea9e5e5f90aa1cfc8ecc97b1908119390bf',
    }],
    ['fatal-attractions', {
      ...
      report: '6eb8e69af4ca5e92c306176b610efe1ea49dbbf0f5c9b1516143403641fbfa3e',
      approval: '566ac5878fe8c188d9d04f8ce0f18fc184329cadb0fa34b4969f375c5d9f3ab1',
    }],
  ]);
  const mappingById = new Map();
