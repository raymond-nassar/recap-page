...
async function buildReportForMapping(mappingPath, peerPaths = [], options = {}) {
  return buildCurrentReportForMapping(mappingPath, peerPaths, {
    ...
    excludedOrderIds: [
      ...
      'falcon-sam-wilson-captain-america-reading-order', 'the-vision-reading-order',
...
test('the character inventory preserves every central disposition, ships forty-four spotlights plus one creator guide, and records five approved reuses', async () => {
  ...
  assert.equal(dispositionCounts.deferred, 73);
  assert.equal(dispositionCounts.excluded, 6);
  assert.equal(dispositionCounts.blocked, 1);
  assert.equal(dispositionCounts['pilot-approved'], 45);
...
test('Adam Warlock publishes the settled source with one exact resolution and three exclusions', async () => {
  ...
  const expectedOrderIds = manifest.lists
    .filter((entry) => entry.id !== id && entry.id !== 'mephisto-reading-order'
      ...
      && !['best-ultron-reading-order', 'winter-soldier-bucky-barnes-reading-order', 'donny-cates-marvel-universe-reading-order-2017', 'falcon-sam-wilson-captain-america-reading-order', 'the-vision-reading-order'].includes(entry.id)
...
test('Silver Surfer settles all four issue #304 gaps without losing source positions', async () => {
  ...
  const expectedOrderIds = manifest.lists
    ...
    .filter((id) => id !== 'silver-surfer-reading-order'
     ...
     && !['best-ultron-reading-order', 'winter-soldier-bucky-barnes-reading-order', 'donny-cates-marvel-universe-reading-order-2017', 'falcon-sam-wilson-captain-america-reading-order', 'the-vision-reading-order'].includes(id)
...
test('the first character batch stays exact through evidence, catalog, and generated data', async () => {
  ...
  assert.equal(characterRuns.length, 54);
  assert.equal(new Set(characterRuns.map((entry) => entry.group ?? entry.id)).size, 53);
});
