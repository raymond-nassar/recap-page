...
test('the character inventory preserves every central disposition, ships forty-two spotlights plus one creator guide, and records five approved reuses', async () => {
  ...
  assert.equal(dispositionCounts.deferred, 75);
  assert.equal(dispositionCounts.excluded, 6);
  assert.equal(dispositionCounts.blocked, 1);
  assert.equal(dispositionCounts['pilot-approved'], 43);
...
test('Adam Warlock publishes the settled source with one exact resolution and three exclusions', async () => {
  ...
  const currentReport = await buildCurrentReportForMapping(
    ...
    [],
    { excludedOrderIds: ['mephisto-reading-order', 'miles-morales-spider-man-reading-order', 'spider-gwen-reading-order', 'best-ultron-reading-order', 'winter-soldier-bucky-barnes-reading-order', 'spider-man-2099-reading-order', 'donny-cates-marvel-universe-reading-order-2017'] },
  );
  const expectedOrderIds = manifest.lists
    .filter((entry) => entry.id !== id && entry.id !== 'mephisto-reading-order'
      ...
      && !['best-ultron-reading-order', 'winter-soldier-bucky-barnes-reading-order', 'donny-cates-marvel-universe-reading-order-2017'].includes(entry.id)
...
test('Silver Surfer settles all four issue #304 gaps without losing source positions', async () => {
  ...
  const expectedOrderIds = manifest.lists
    ...
    .filter((id) => id !== 'silver-surfer-reading-order'
     ...
     && !['best-ultron-reading-order', 'winter-soldier-bucky-barnes-reading-order', 'donny-cates-marvel-universe-reading-order-2017'].includes(id)
...
test('the frozen Star-Lord evidence stays complete, fresh, distinct, and exact', async () => {
  ...
  const starLordChronology = manifest.lists.filter((entry) => !['best-ultron-reading-order', 'miles-morales-spider-man-reading-order', 'spider-gwen-reading-order', 'spider-man-2099-reading-order', 'winter-soldier-bucky-barnes-reading-order', 'donny-cates-marvel-universe-reading-order-2017'].includes(entry.id));
...
test('the first character batch stays exact through evidence, catalog, and generated data', async () => {
  ...
  assert.equal(catalog.lists.length, 258);
  const characterRuns = catalog.lists.filter((entry) => entry.type === 'character-run');
  assert.equal(characterRuns.length, 52);
  assert.equal(new Set(characterRuns.map((entry) => entry.group ?? entry.id)).size, 51);
});
