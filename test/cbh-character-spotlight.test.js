...
const laterCbhIds = [...laterMcuIds, 'ultimate-marvel-intro'];
...
test('the frozen White Tiger evidence stays exact through every generated surface', async () => {
  ...
  const reviewedLibraryDigest = await prePublicationLibraryDigest(
    manifest,
    [...characterCandidateIds, ...laterHistoricalIds, ...laterCbhIds, ...continuationHistoricalIds],
  );
...
test('the frozen Rocket evidence stays complete, fresh, and exact through every generated surface', async () => {
  ...
  assert.deepEqual(
    regeneratedReport.comparisons.filter((comparison) => (
      ![grootCandidateId, starLordCandidateId, ...laterHistoricalIds, ...laterCbhIds, ...continuationHistoricalIds]
        .includes(comparison.orderId)
    )),
    report.comparisons,
  );
...
test('the frozen Groot evidence stays complete, fresh, distinct, and exact', async () => {
  ...
  const reviewedLibraryDigest = await prePublicationLibraryDigest(
    manifest,
    [grootCandidateId, cosmicCandidateId, ...laterCbhIds, ...continuationHistoricalIds],
  );
  const regeneratedReport = await buildReportForMapping(
  ...
  assert.deepEqual(
    regeneratedReport.comparisons.filter((comparison) => !laterCbhIds.includes(comparison.orderId)
...
test('the frozen Star-Lord evidence stays complete, fresh, distinct, and exact', async () => {
  ...
  const reviewedLibraryDigest = await prePublicationLibraryDigest(
    manifest,
    [starLordCandidateId, cosmicCandidateId, grootCandidateId, ...laterCbhIds, ...continuationHistoricalIds],
  );
  const regeneratedReport = await buildReportForMapping(
