...
const marvelKnightsToPlanetXId = 'marvel-knights-to-planet-x';
...
function magnetoSourceOrderDigest(occurrences) {
  const sourceOrder = occurrences.map((entry) => ({
    sourceGroup: entry.sourceGroup,
    sourceRangeReference: entry.sourceRangeReference,
    sourceTextClause: entry.sourceTextClause,
    sourceIssueReference: entry.sourceIssueReference,
    normalizedSeriesTitle: entry.normalizedSeriesTitle ?? null,
    seriesYear: entry.seriesYear ?? null,
    issueNumber: entry.issueNumber ?? null,
    sourceIdentity: entry.sourceIdentity ?? null,
    disposition: entry.disposition,
    canonicalSourcePosition: entry.canonicalSourcePosition ?? null,
    sourceNode: entry.sourceNode,
  }));
  return createHash('sha256').update(JSON.stringify(sourceOrder)).digest('hex');
}

function validateMagnetoLedger(ledger) {
  const { occurrences } = ledger;
  assert.equal(ledger.ledgerStatus, 'offline-provisional');
  assert.equal(Array.isArray(occurrences), true);
  assert.equal(ledger.sourceOccurrenceCount, 811);
  assert.equal(occurrences.length, 811);

  const positions = occurrences.map((entry) => entry.sourcePosition);
  assert.deepEqual(positions, Array.from({ length: 811 }, (_, index) => index + 1));
  ...
  assert.deepEqual(counts, {
    'canonical-candidate': 769,
    repeat: 31,
    exclusion: 11,
  });
  assert.deepEqual(ledger.counts, {
    canonicalCandidates: 769,
    repeats: 31,
    exclusions: 11,
  });
  assert.equal(counts['canonical-candidate'] + counts.repeat + counts.exclusion, 811);
  assert.equal(
    magnetoSourceOrderDigest(occurrences),
    ledger.sourceOrderSha256,
    'source-order entries must retain their source identity, group, and lexical order',
  );
  assert.equal(occurrences.filter((entry) => entry.disposition !== 'exclusion').length, 800);
  assert.equal(
    occurrences.filter((entry) => entry.disposition !== 'exclusion' && entry.issueNumber !== null).length,
    784,
  );
  assert.equal(
    occurrences.filter((entry) => entry.disposition === 'canonical-candidate'
      && entry.issueNumber === null).length,
    16,
  );
  assert.equal(
    occurrences.filter((entry) => entry.evidence?.localSeriesEvidence).length,
    11,
  );
  assert.equal(
    occurrences.filter((entry) => entry.disposition !== 'exclusion').every((entry) => (
      entry.seriesId === null
      && entry.candidateIssueId === null
      && entry.manualSeriesSelectionApproved === false
      && entry.provisionalResolution === 'provider-unverified'
    )),
    true,
  );
  assert.equal(
    occurrences.filter((entry) => entry.disposition === 'exclusion')
      .every((entry) => entry.provisionalResolution === 'not-an-issue'),
    true,
  );
  ...
  assert.equal(
    occurrences.every((entry) => typeof entry.sourceGroup === 'string'
      ...
      && Number.isInteger(entry.sourceNode?.contentIndex)
      && typeof entry.sourceNode.kind === 'string'
      && typeof entry.sourceNode.text === 'string'),
    true,
  );
  assert.equal(
    occurrences.every((entry) => entry.sourceRangeReference === null
      || (typeof entry.sourceRangeReference === 'string'
        && entry.sourceRangeReference.trim().length > 0)),
    true,
  );
  assert.deepEqual(
    [...new Set(occurrences.map((entry) => entry.sourceGroup))],
    [
  ...
  assert.equal(
    occurrences.every((entry, index) => index === 0
      || entry.sourceNode.contentIndex >= occurrences[index - 1].sourceNode.contentIndex),
    true,
  );
  assert.equal(
    occurrences.filter((entry) => entry.sourceNode.kind === 'collection-clause')
      .every((entry) => entry.sourceNode.text === 'Collects'),
    true,
  );
  assert.equal(occurrences.filter((entry) => entry.sourceNode.kind === 'collection-clause').length, 750);
  assert.equal(occurrences.filter((entry) => entry.sourceNode.kind === 'commentary').length, 35);
  assert.equal(occurrences.filter((entry) => entry.sourceNode.kind === 'direct-issue-list').length, 25);
  assert.equal(occurrences.filter((entry) => entry.sourceNode.kind === 'standalone-issue-link').length, 1);
  assert.deepEqual(
    [...new Set(
      occurrences
        .filter((entry) => entry.sourceNode.kind === 'direct-issue-list')
        .map((entry) => entry.sourceNode.text),
    )],
    [
  ...
  assert.equal(
    new Set([...canonicalByPosition.values()].map((entry) => entry.sourceIdentity)).size,
    canonicalByPosition.size,
  );
  for (const repeat of occurrences.filter((entry) => entry.disposition === 'repeat')) {
    const canonical = canonicalByPosition.get(repeat.canonicalSourcePosition);
    assert.ok(canonical);
    assert.ok(repeat.canonicalSourcePosition < repeat.sourcePosition);
    assert.equal(repeat.sourceIdentity, canonical.sourceIdentity);
  }
}
...
test('the Deadpool Best of guide preserves its source groups, repeats, metadata gaps, and complete-library approval', async () => {
  ...
  assert.deepEqual(
    ...
    [{
      orderId: 'wolverine-reading-order',
      sharedCount: 2,
      sharedIds: ['32573', '32581'],
...
test('Magneto keeps the document-ordered offline boundary and accounting summary', async () => {
  const inventory = await readJson('scripts/data/cbh-character-inventory.json');
  ...
  assert.equal(record.sourceContentSha256, 'eee0f0eedf0e3d1350e54410d6ee80f0fd0b56d9491a36c3700ea429d9d61078');
  assert.match(record.reason, /811 document-ordered source positions/);
  assert.match(record.reason, /769 canonical candidates, 31 repeats, and 11 exclusions/);
});

test('Magneto source ledger preserves document order and offline source evidence', async () => {
  ...
  assert.equal(
    ledger.sourceBoundary,
    'Full entry-content document order. Heading scan contains no Best Comics or Essential Comics section; Essential X-Men names collection titles only.',
  );
  assert.equal(ledger.sourceRetrievedAt, '2026-08-27');
  assert.equal(
    ledger.sourceContentNormalization,
    'NFC UTF-8 join of entry-content h2, h3, and p text in document order, with each node prefixed by its tag name and whitespace collapsed to one space.',
  );
  assert.equal(ledger.sourceContentSha256, 'eee0f0eedf0e3d1350e54410d6ee80f0fd0b56d9491a36c3700ea429d9d61078');
  assert.deepEqual(ledger.sourceExtraction, {
    basis: 'Rows are emitted directly from the permitted article nodes before offline disposition classification.',
    selector: 'entry-content h2, h3, p',
    sourceNodeCount: 297,
    sourceOrder: 'sourceNode.contentIndex followed by lexical issue order within that node',
    markers: {
      'collection-clause': 'The source paragraph begins with Collects.',
      commentary: 'The source paragraph names an issue or is retained as non-issue commentary.',
      'direct-issue-list': 'A preceding source marker names a list of individual issue paragraphs.',
      'standalone-issue-link': 'A linked source paragraph itself names one issue.',
    },
  });
  validateMagnetoLedger(ledger);

  const trial = ledger.occurrences[0];
  assert.equal(trial.sourceIssueReference, 'Uncanny X-Men #200');
  assert.equal(trial.disposition, 'canonical-candidate');
  assert.match(trial.sourceTextClause, /The Trial of Magneto/);
  const originalDisposition = trial.disposition;
  trial.disposition = 'exclusion';
  try {
    assert.throws(() => validateMagnetoLedger(ledger), /canonical-candidate/);
  } finally {
    trial.disposition = originalDisposition;
  }

  const priorSourceOccurrenceCount = ledger.sourceOccurrenceCount;
  const priorRows = ledger.occurrences.splice(736);
  ledger.sourceOccurrenceCount = 736;
  try {
    assert.throws(() => validateMagnetoLedger(ledger), /811/);
  } finally {
    ledger.sourceOccurrenceCount = priorSourceOccurrenceCount;
    ledger.occurrences.push(...priorRows);
  }

  const deleted = ledger.occurrences.splice(400, 1);
  try {
    assert.throws(() => validateMagnetoLedger(ledger), /811/);
  } finally {
    ledger.occurrences.splice(400, 0, ...deleted);
  }

  const originalOrder = ledger.occurrences.splice(0, ledger.occurrences.length);
  const originalPositions = originalOrder.map((entry) => entry.sourcePosition);
  ledger.occurrences.push(
    ...originalOrder.slice().sort((left, right) => left.disposition.localeCompare(right.disposition)),
  );
  ledger.occurrences.forEach((entry, index) => {
    entry.sourcePosition = index + 1;
  });
  try {
    assert.throws(() => validateMagnetoLedger(ledger));
  } finally {
    ledger.occurrences.splice(0, ledger.occurrences.length, ...originalOrder);
    ledger.occurrences.forEach((entry, index) => {
      entry.sourcePosition = originalPositions[index];
    });
  }

  [ledger.occurrences[0], ledger.occurrences[1]] = [
    ledger.occurrences[1],
    ledger.occurrences[0],
  ];
  try {
    assert.throws(() => validateMagnetoLedger(ledger));
  } finally {
  ...
  const sameNode = ledger.occurrences
    .map((entry, index) => ({ entry, index }))
    .find(({ entry }, index, entries) => index > 0
      && entry.sourceNode.contentIndex === entries[index - 1].entry.sourceNode.contentIndex);
  assert.ok(sameNode);
  const sameNodePrior = sameNode.index - 1;
  const originalSameNodePositions = ledger.occurrences.map((entry) => entry.sourcePosition);
  [ledger.occurrences[sameNodePrior], ledger.occurrences[sameNode.index]] = [
    ledger.occurrences[sameNode.index],
    ledger.occurrences[sameNodePrior],
  ];
  ledger.occurrences.forEach((entry, index) => {
    entry.sourcePosition = index + 1;
  });
  try {
    assert.throws(() => validateMagnetoLedger(ledger), /source-order entries/);
  } finally {
    [ledger.occurrences[sameNodePrior], ledger.occurrences[sameNode.index]] = [
      ledger.occurrences[sameNode.index],
      ledger.occurrences[sameNodePrior],
    ...
    ledger.occurrences.forEach((entry, index) => {
      entry.sourcePosition = originalSameNodePositions[index];
    });
  }

  const originalSecondPosition = ledger.occurrences[1].sourcePosition;
  ledger.occurrences[1].sourcePosition = ledger.occurrences[0].sourcePosition;
  try {
    assert.throws(() => validateMagnetoLedger(ledger));
  } finally {
    ledger.occurrences[1].sourcePosition = originalSecondPosition;
  }
  ...
  for (const reference of formerGaps) {
    const entry = ledger.occurrences.find((candidate) => candidate.sourceIssueReference === reference);
    assert.ok(entry);
    assert.equal(entry.disposition, 'canonical-candidate');
    assert.ok(entry.evidence.localSeriesEvidence);
  }
  const locallyEvidencedAdditions = [
    'X-Men: God Loves, Man Kills',
    'X-Men: Magneto War One-Shot',
    'Age Of X Alpha',
  ...
  assert.equal(locallyEvidencedAdditions.length, 8);
  assert.equal(
    locallyEvidencedAdditions.every((reference) => (
      ledger.occurrences.some((entry) => entry.sourceIssueReference === reference
        && entry.disposition === 'canonical-candidate'
        && entry.evidence.localSeriesEvidence)
    )),
    true,
  );
  assert.equal(ledger.occurrences.some((entry) => entry.disposition === 'gap'), false);
  assert.equal(
    ledger.occurrences.some((entry) => ['2001', '2002', '2003', '2004'].includes(entry.issueNumber)),
    false,
  );
  assert.equal(
    ledger.occurrences.find((entry) => entry.sourceIssueReference === 'X-Men Legacy #249').normalizedSeriesTitle,
    'X-Men Legacy',
  );
  assert.equal(
    ledger.occurrences.find((entry) => entry.sourceIssueReference === 'S.W.O.R.D. by Ewing #6').normalizedSeriesTitle,
    'S.W.O.R.D.',
  );
  assert.deepEqual(
    ledger.occurrences
      .filter((entry) => entry.sourceRangeReference === 'X-Men by Hickman'
        && entry.sourceIssueReference.startsWith('X-Men (2019)'))
      .map((entry) => entry.issueNumber),
    [...issueRange('X-Men', 2019, 1, 11), ...issueRange('X-Men', 2019, 16, 21)]
...
test('the Modern X-Men fast-track preserves its selected source boundary and overlaps', async () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      { orderId: 'deadpool-best-of', relationship: 'partial', sharedCount: 2 },
...
test('Venom preserves every source occurrence through its published guide', async () => {
  ...
  const reviewedLibraryDigest = await prePublicationLibraryDigest(manifest, [venomCandidateId]);
