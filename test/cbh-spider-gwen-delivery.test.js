...
test('Spider-Gwen digests and approval preserve the reviewed source library', async () => {
  ...
  assert.deepEqual(report.comparisons.map((row) => row.orderId).sort(),
    manifest.lists.filter((row) => row.id !== id
      ...
      && !['best-ultron-reading-order', 'winter-soldier-bucky-barnes-reading-order', 'donny-cates-marvel-universe-reading-order-2017', 'falcon-sam-wilson-captain-america-reading-order', 'the-vision-reading-order', 'emma-frost-reading-order', 'doctor-octopus-otto-octavius-reading-order'].includes(row.id) && row.id !== 'spider-man-2099-reading-order').map((row) => row.id).sort());
