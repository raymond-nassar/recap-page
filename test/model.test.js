...
test('the bundled orders carry a gap the payload field never reported', () => {
  ...
  // Surfer adds four source-preserving metadata gaps. Moon Knight adds 18 more.
  assert.equal(claimed, 1327, 'the payload placeholder total moved; re-derive the figures in the record');
  assert.equal(placeholders, 1327, 'the bundled placeholder total moved; re-derive the figures in the record');
...
test('the bundled orders really do contain issues no lookup can answer for', () => {
  ...
  // Surfer adds four. Moon Knight adds 18.
  assert.equal(refused.length, 1371);
