...
test('Falcon approval preserves every source-manifest order in its reviewed library', async () => {
  ...
  const current = await buildReportForMapping(
    `scripts/data/cbh-mappings/${id}.json`, [], { excludedOrderIds: ['donny-cates-marvel-universe-reading-order-2017', 'the-vision-reading-order', 'emma-frost-reading-order', 'doctor-octopus-otto-octavius-reading-order'] },
  );
  ...
  assert.deepEqual(new Set(report.comparisons.map((row) => row.orderId)),
    new Set(manifest.lists.filter((row) => row.id !== id
      && row.id !== 'donny-cates-marvel-universe-reading-order-2017' && row.id !== 'the-vision-reading-order' && row.id !== 'emma-frost-reading-order' && row.id !== 'doctor-octopus-otto-octavius-reading-order').map((row) => row.id)));
