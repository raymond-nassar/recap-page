...
test('no shipped reading order carries Marvel description prose', async () => {
  ...
  assert.equal(orders.length, 156, `the catalog lists ${orders.length} orders, not 156, so this test's coverage has changed`);
...
// The test above reads the catalog, so it sees exactly the one hundred and forty-eight files the catalog names and
...
test('no file outside node_modules, .git and .copilot-tracking carries an items[].description string', async () => {
  ...
  assert.equal(scanned, 161, `${scanned} item-bearing files were found, not 161, so this test's coverage has changed`);
