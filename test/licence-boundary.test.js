...
test('no shipped reading order carries Marvel description prose', async () => {
  ...
  assert.equal(orders.length, 86, `the catalog lists ${orders.length} orders, not 86, so this test's coverage has changed`);
...
// The test above reads the catalog, so it sees exactly the eighty-six files the catalog names and
...
// leaves the population with nothing said. A floor of eighty-six still passes at that point, because
// the catalog's own files alone clear it. An exact eighty-nine does not.
//
...
test('no file outside node_modules, .git and .copilot-tracking carries an items[].description string', async () => {
  ...
  assert.equal(scanned, 89, `${scanned} item-bearing files were found, not 89, so this test's coverage has changed`);
