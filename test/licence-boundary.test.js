...
test('no shipped reading order carries Marvel description prose', async () => {
  ...
  assert.equal(orders.length, 87, `the catalog lists ${orders.length} orders, not 87, so this test's coverage has changed`);
...
// The test above reads the catalog, so it sees exactly the eighty-seven files the catalog names and
...
// leaves the population with nothing said. A floor of eighty-seven still passes at that point, because
// the catalog's own files alone clear it. An exact ninety does not.
//
...
test('no file outside node_modules, .git and .copilot-tracking carries an items[].description string', async () => {
  ...
  assert.equal(scanned, 90, `${scanned} item-bearing files were found, not 90, so this test's coverage has changed`);
