...
test('no shipped reading order carries Marvel description prose', async () => {
  ...
  assert.equal(orders.length, 96, `the catalog lists ${orders.length} orders, not 96, so this test's coverage has changed`);
...
// The test above reads the catalog, so it sees exactly the ninety-six files the catalog names and
...
// leaves the population with nothing said. A floor of ninety-six still passes at that point, because
// the catalog's own files alone clear it. An exact ninety-nine does not.
//
...
test('no file outside node_modules, .git and .copilot-tracking carries an items[].description string', async () => {
  ...
  assert.equal(scanned, 99, `${scanned} item-bearing files were found, not 99, so this test's coverage has changed`);
