...
test('no shipped reading order carries Marvel description prose', async () => {
  ...
  assert.equal(orders.length, 159, `the catalog lists ${orders.length} orders, not 159, so this test's coverage has changed`);
...
test('no file outside node_modules, .git and .copilot-tracking carries an items[].description string', async () => {
  ...
  assert.equal(scanned, 164, `${scanned} item-bearing files were found, not 164, so this test's coverage has changed`);
