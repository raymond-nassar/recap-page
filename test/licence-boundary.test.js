...
test('no shipped reading order carries Marvel description prose', async () => {
  ...
  assert.equal(orders.length, 143, `the catalog lists ${orders.length} orders, not 143, so this test's coverage has changed`);
...
// The test above reads the catalog, so it sees exactly the one hundred and forty-three files the catalog names and
...
// leaves the population with nothing said. A floor of one hundred and forty-two still passes at that point,
// because the catalog's own files alone clear it. An exact one hundred and forty-six does not.
//
...
test('no file outside node_modules, .git and .copilot-tracking carries an items[].description string', async () => {
  ...
  assert.equal(scanned, 146, `${scanned} item-bearing files were found, not 146, so this test's coverage has changed`);
