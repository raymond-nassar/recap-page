...
test('no shipped reading order carries Marvel description prose', async () => {
  ...
  assert.equal(orders.length, 256, `the catalog lists ${orders.length} orders, not 256, so this test's coverage has changed`);
...
test('no file outside node_modules, .git and .copilot-tracking carries an items[].description string', async () => {
  ...
  assert.equal(scanned, 260, `${scanned} item-bearing files were found, not 260, so this test's coverage has changed`);
