...
test('no shipped reading order carries Marvel description prose', async () => {
  ...
  assert.equal(orders.length, 147, `the catalog lists ${orders.length} orders, not 147, so this test's coverage has changed`);
...
// because the catalog's own files alone clear it. An exact one hundred and fifty does not.
//
...
test('no file outside node_modules, .git and .copilot-tracking carries an items[].description string', async () => {
  ...
  assert.equal(scanned, 152, `${scanned} item-bearing files were found, not 152, so this test's coverage has changed`);
