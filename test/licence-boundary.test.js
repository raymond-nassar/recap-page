...
test('no shipped reading order carries Marvel description prose', async () => {
  ...
  assert.equal(orders.length, 135, `the catalog lists ${orders.length} orders, not 135, so this test's coverage has changed`);
...
// The test above reads the catalog, so it sees exactly the one hundred and thirty-five files the catalog names and
...
// leaves the population with nothing said. A floor of one hundred and thirty-five still passes at that point,
// because the catalog's own files alone clear it. An exact one hundred and thirty-eight does not.
//
...
test('no file outside node_modules, .git and .copilot-tracking carries an items[].description string', async () => {
  ...
  assert.equal(scanned, 138, `${scanned} item-bearing files were found, not 138, so this test's coverage has changed`);
