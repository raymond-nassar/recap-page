...
test('no shipped reading order carries Marvel description prose', async () => {
  ...
  assert.equal(orders.length, 117, `the catalog lists ${orders.length} orders, not 117, so this test's coverage has changed`);
...
test('no file outside node_modules, .git and .copilot-tracking carries an items[].description string', async () => {
  ...
  assert.equal(scanned, 120, `${scanned} item-bearing files were found, not 120, so this test's coverage has changed`);
