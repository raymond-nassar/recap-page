...
test('no shipped reading order carries Marvel description prose', async () => {
  ...
  assert.equal(orders.length, 155, `the catalog lists ${orders.length} orders, not 155, so this test's coverage has changed`);
...
test('no file outside node_modules, .git and .copilot-tracking carries an items[].description string', async () => {
  ...
  assert.equal(scanned, 160, `${scanned} item-bearing files were found, not 160, so this test's coverage has changed`);
