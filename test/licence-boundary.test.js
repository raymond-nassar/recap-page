...
test('no shipped reading order carries Marvel description prose', async () => {
  ...
  assert.equal(orders.length, 251, `the catalog lists ${orders.length} orders, not 251, so this test's coverage has changed`);
...
// The test above reads the catalog, so it sees exactly the two hundred and fifty-one files the catalog names and
...
// that point, because the catalog's own files alone clear it. An exact two hundred and fifty-five
// does not.
//
...
test('no file outside node_modules, .git and .copilot-tracking carries an items[].description string', async () => {
  ...
  assert.equal(scanned, 255, `${scanned} item-bearing files were found, not 255, so this test's coverage has changed`);
