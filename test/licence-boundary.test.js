...
test('no shipped reading order carries Marvel description prose', async () => {
  ...
  assert.equal(orders.length, 95, `the catalog lists ${orders.length} orders, not 95, so this test's coverage has changed`);
...
// The test above reads the catalog, so it sees exactly the ninety-five files the catalog names and
...
// leaves the population with nothing said. A floor of ninety-five still passes at that point, because
// the catalog's own files alone clear it. An exact ninety-eight does not.
//
...
test('no file outside node_modules, .git and .copilot-tracking carries an items[].description string', async () => {
  ...
  assert.equal(scanned, 98, `${scanned} item-bearing files were found, not 98, so this test's coverage has changed`);
