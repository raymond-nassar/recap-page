...
test('no shipped reading order carries Marvel description prose', async () => {
  ...
  assert.equal(orders.length, 259, `the catalog lists ${orders.length} orders, not 259, so this test's coverage has changed`);
...
// The test above reads the catalog, so it sees exactly the two hundred and fifty-nine files the catalog names and
...
// that point, because the catalog's own files alone clear it. An exact two hundred and sixty-three
// does not.
//
...
test('no file outside node_modules, .git and .copilot-tracking carries an items[].description string', async () => {
  ...
  assert.equal(scanned, 263, `${scanned} item-bearing files were found, not 263, so this test's coverage has changed`);
