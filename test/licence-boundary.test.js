...
test('no shipped reading order carries Marvel description prose', async () => {
  ...
  assert.equal(orders.length, 161, `the catalog lists ${orders.length} orders, not 161, so this test's coverage has changed`);
...
test('no file outside node_modules, .git and .copilot-tracking carries an items[].description string', async () => {
  ...
  assert.equal(scanned, 166, `${scanned} item-bearing files were found, not 166, so this test's coverage has changed`);
