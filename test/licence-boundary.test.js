...
test('no shipped reading order carries Marvel description prose', async () => {
  ...
  assert.equal(orders.length, 260, `the catalog lists ${orders.length} orders, not 260, so this test's coverage has changed`);
...
// The test above reads the catalog, so it sees exactly the two hundred and sixty files the catalog names and
...
// that point, because the catalog's own files alone clear it. An exact two hundred and sixty-four
// does not.
//
...
test('no file outside node_modules, .git and .copilot-tracking carries an items[].description string', async () => {
  ...
  assert.equal(scanned, 264, `${scanned} item-bearing files were found, not 264, so this test's coverage has changed`);
