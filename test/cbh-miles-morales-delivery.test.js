...
test('Miles approvals preserve the reviewed library and frozen evidence', async () => {
  ...
  assert.doesNotThrow(() => assertApprovedRelationshipReview({
    ...
    expectedOrderIds: manifest.lists.filter((row) => row.id !== id && row.id !== 'the-vision-reading-order' && row.id !== 'emma-frost-reading-order' && row.id !== 'doctor-octopus-otto-octavius-reading-order').map((row) => row.id),
  }));
});
