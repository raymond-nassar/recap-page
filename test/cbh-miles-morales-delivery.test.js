...
test('Miles approvals preserve the reviewed library and frozen evidence', async () => {
  ...
  const current = await buildReportForMapping(
    `scripts/data/cbh-mappings/${id}.json`, [], { excludedOrderIds: ['best-ultron-reading-order', 'winter-soldier-bucky-barnes-reading-order', 'spider-man-2099-reading-order', 'donny-cates-marvel-universe-reading-order-2017', 'falcon-sam-wilson-captain-america-reading-order', 'the-vision-reading-order'] },
  );
  ...
  assert.doesNotThrow(() => assertApprovedRelationshipReview({
    ...
    expectedOrderIds: manifest.lists.filter((row) => row.id !== id
      && !['best-ultron-reading-order', 'winter-soldier-bucky-barnes-reading-order', 'donny-cates-marvel-universe-reading-order-2017', 'falcon-sam-wilson-captain-america-reading-order', 'the-vision-reading-order'].includes(row.id) && row.id !== 'spider-man-2099-reading-order').map((row) => row.id),
  }));
});
