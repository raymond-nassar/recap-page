...
| C3 | Relationship approval binds the fresh packet, mapping, report, library digest, every selected-peer mapping digest, and one central disposition for every comparison. | `scripts/author-cbh-packet.mjs:298-375` | read | high | Non-none relationships require human or stronger-model authority. |
...
| C5 | Authoring validates every fresh evidence layer, then creates one ungrouped manifest entry and exact Markdown order without altering existing guide data. | `scripts/author-cbh-packet.mjs:394-501` | read | high | A one-guide packet avoids the same-batch duplicate issue-id guard. |
| C6 | Packet, mapping, report, and approval validators reject stale digests, and library hashing deliberately ignores editorial `spotlightKind` so taxonomy updates do not rewrite issue-library evidence. | `scripts/lib/cbh-inventory.mjs:192-265` | read | high | Negative stale tests already have shared helpers. |
