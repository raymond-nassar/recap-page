...
export const PAIRS = [
  ...
  // `.railed .rail-foot .pill` sets `color: transparent` at `src/styles.css:528-529`, so this is
...
export const SURFACES = {
  ...
  'the ghost button on the unreadable-data banner': {
    ...
    css: 'rgb(var(--tint-base) / 5.882%) over the banner, at `src/styles.css:813`',
  },
};
...
// stated as text beside it, at `src/js/main.js:1613` in the rail and `src/js/views/shared/saved-lists.js:30` in the
// saved lists.
//
...
export const KNOWN = [
  ...
  // the button takes its accessible name from the `aria-label` at   `src/js/views/reading.js:667`, which
  // replaces the glyph in the name computation, and `aria-pressed` at   `src/js/views/reading.js:666`
