...
`src/js/main.js:3355-3369` hands the store a function; the function itself, at
...
announcement can be gated on the outcome: `src/js/main.js:367-369` speaks only if the write
...
`src/js/main.js:3883-3983`. The first nonempty page creates and fills its list inside one Store
update, and every later completed page uses the same boundary, at `src/js/main.js:3840-3881` and
`src/js/main.js:3995-4022`. Cancelling retires that owner before aborting its request, so a response
...
`src/js/main.js:4062-4093`. A stop before the first page creates no list, while every page already
