...
itself is loaded at `src/index.html:1041`. The launch page, which is the tab a reader's issue opens
...
Those five are built together at `src/js/main.js:91-106`. Read that block and you have read the
...
`src/js/main.js:5899-5901`. The hydrator itself is not rebuilt; only its reference to the client is
...
`src/js/main.js:3353-3367` hands the store a function; the function itself, at
...
announcement can be gated on the outcome: `src/js/main.js:366-368` speaks only if the write
...
surfaces, the six screens plus the blocked banner, at `src/js/main.js:6241-6262`, but the reading
...
by identity afterwards, at `src/js/main.js:3248`, which is what keeps the keyboard where the reader
left it. The row list is committed by moving nodes rather than replacing the container, at
`src/js/main.js:3136-3144`.
...
`src/js/main.js:3881-3981`. The first nonempty page creates and fills its list inside one Store
update, and every later completed page uses the same boundary, at `src/js/main.js:3838-3879` and
`src/js/main.js:3993-4020`. Cancelling retires that owner before aborting its request, so a response
...
`src/js/main.js:4060-4091`. A stop before the first page creates no list, while every page already
...
`src/js/main.js:59-61`, and the response cache is not in `localStorage` at all.
...
| `mrt.settings` | the settings form, the cover art switch, the theme control and the reading filter, at `src/js/main.js:737-745` | nothing | Preferences, not data. Deliberately outside the state so a settings write can never fail a progress write. An older `cachePurge` field is read once as migration input but is no longer authoritative or written by current code. |
| `mrt.cache-purge.v1` | successful cache cleanup, at `src/js/main.js:697-715` | nothing | A monotonic cleanup generation held apart from settings so an older tab cannot lower it by serializing the settings shape it knows. Current tabs serialize its read-max-write step through one origin-wide browser lock. |
| `sidebar.collapsed` | the sidebar toggle, at `src/js/main.js:1273` | nothing | Whether the rail is collapsed. Wrapped in its own try, because losing it is not worth an error. |
