...
`src/js/main.js:4628-4630`. The hydrator itself is not rebuilt; only its reference to the client is
...
`src/js/main.js:2668-2671` hands the store a function; the function itself, at
...
surfaces, the six screens plus the blocked banner, at `src/js/main.js:4955-4975`, but the reading
...
by identity afterwards, at `src/js/main.js:2567`, which is what keeps the keyboard where the reader
left it. The row list is committed by moving nodes rather than replacing the container, at
`src/js/main.js:2455-2463`.
...
| `mrt.settings` | the settings form, the cover art switch, the theme control and the reading filter, at `src/js/main.js:630` | nothing | Preferences, not data. Deliberately outside the state so a settings write can never fail a progress write. |
| `sidebar.collapsed` | the sidebar toggle, at `src/js/main.js:970` | nothing | Whether the rail is collapsed. Wrapped in its own try, because losing it is not worth an error. |
