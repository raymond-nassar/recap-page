...
`src/js/main.js:5888-5890`. The hydrator itself is not rebuilt; only its reference to the client is
...
`src/js/main.js:3350-3364` hands the store a function; the function itself, at
...
announcement can be gated on the outcome: `src/js/main.js:363-365` speaks only if the write
...
surfaces, the six screens plus the blocked banner, at `src/js/main.js:6230-6251`, but the reading
...
by identity afterwards, at `src/js/main.js:3245`, which is what keeps the keyboard where the reader
left it. The row list is committed by moving nodes rather than replacing the container, at
`src/js/main.js:3133-3141`.
...
`src/js/main.js:3878-3978`. The first nonempty page creates and fills its list inside one Store
update, and every later completed page uses the same boundary, at `src/js/main.js:3835-3876` and
`src/js/main.js:3990-4017`. Cancelling retires that owner before aborting its request, so a response
...
`src/js/main.js:4057-4088`. A stop before the first page creates no list, while every page already
...
| `mrt.settings` | the settings form, the cover art switch, the theme control and the reading filter, at `src/js/main.js:734-742` | nothing | Preferences, not data. Deliberately outside the state so a settings write can never fail a progress write. An older `cachePurge` field is read once as migration input but is no longer authoritative or written by current code. |
| `mrt.cache-purge.v1` | successful cache cleanup, at `src/js/main.js:694-712` | nothing | A monotonic cleanup generation held apart from settings so an older tab cannot lower it by serializing the settings shape it knows. Current tabs serialize its read-max-write step through one origin-wide browser lock. |
| `sidebar.collapsed` | the sidebar toggle, at `src/js/main.js:1270` | nothing | Whether the rail is collapsed. Wrapped in its own try, because losing it is not worth an error. |
