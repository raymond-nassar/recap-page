...
itself is loaded at `src/index.html:1095`. The launch page, which is the tab a reader's issue opens
...
`src/js/main.js:2061-2076`. An in-flight synopsis run is cancelled and its tab-memory prose is
...
`src/js/main.js:1799-1809`, so browsing does not depend on the metadata service.
...
`src/js/views/reading.js:761-763` hands the store a function; the function itself, at
...
refresh fan-out at `src/js/main.js:2492-2512`, including the rail, reading view, Home, Library hub
...
`src/js/views/reading.js:665`, which is what keeps the keyboard where the reader left it. The row list is
...
| `mrt.settings` | the settings form, cover art, theme, D shortcut, description hiding and reading filter controls, at `src/js/main.js:639-650` | nothing | Preferences, not data, and excluded from reading-progress backups and restores. Deliberately outside the state so a settings write can never fail a progress write. D shortcut and description hiding default on; failed saves apply to this tab with a visible reload warning. Description hiding stores only a boolean: actual changes reset individual disclosure choices, not tab-held prose, and never fetch. An older `cachePurge` field is read once as migration input but is no longer authoritative or written by current code. |
| `mrt.cache-purge.v1` | successful cache cleanup, at `src/js/main.js:608-626` | nothing | A monotonic cleanup generation held apart from settings so an older tab cannot lower it by serializing the settings shape it knows. Current tabs serialize its read-max-write step through one origin-wide browser lock. |
| `sidebar.collapsed` | deliberate desktop sidebar toggles, inside the persist guard at `src/js/main.js:1116-1123` | nothing | Whether the desktop rail is compact. Narrow open and closed state is ephemeral and never writes this key. Wrapped in its own try, because losing it is not worth an error. |
...
`src/js/main.js:3080-3131`. The selected id lives only in the validated `path` query of the hash
...
`src/js/main.js:2991-3023`, while the existing Store-driven render path calls the position-only
refresh at `src/js/main.js:2492-2512`. That refresh leaves cards, controls, focus, scroll and
