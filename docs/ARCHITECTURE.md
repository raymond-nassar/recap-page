...
`src/js/main.js:82-102`; application-wide bookkeeping lives at module scope, while constructed view
...
`src/js/main.js:3224-3234`. An in-flight synopsis run is cancelled and its tab-memory prose is
...
`src/js/main.js:2983-2993`, so browsing does not depend on the metadata service.
...
`src/js/main.js:2547-2561` hands the store a function; the function itself, at
...
refresh fan-out at `src/js/main.js:3467-3486`, including the rail, reading view, Home, Library hub
...
`src/js/main.js:2442`, which is what keeps the keyboard where the reader left it. The row list is
committed by moving nodes rather than replacing the container, at
`src/js/main.js:2330-2338`.
...
| `mrt.settings` | the settings form, the cover art switch, the theme control and the reading filter, at `src/js/main.js:648-653` | nothing | Preferences, not data. Deliberately outside the state so a settings write can never fail a progress write. An older `cachePurge` field is read once as migration input but is no longer authoritative or written by current code. |
| `mrt.cache-purge.v1` | successful cache cleanup, at `src/js/main.js:617-635` | nothing | A monotonic cleanup generation held apart from settings so an older tab cannot lower it by serializing the settings shape it knows. Current tabs serialize its read-max-write step through one origin-wide browser lock. |
| `sidebar.collapsed` | the sidebar toggle, at `src/js/main.js:1040` | nothing | Whether the rail is collapsed. Wrapped in its own try, because losing it is not worth an error. |
...
`src/js/main.js:3987-4021`. The selected id lives only in the validated `path` query of the hash
...
`src/js/main.js:119-139`; that repaint changes only the progress outputs at
