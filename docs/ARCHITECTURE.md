...
Those five are built together at `src/js/main.js:84-99`. Read that block and you have read the
...
`src/js/main.js:5177-5179`. The hydrator itself is not rebuilt; only its reference to the client is
...
`src/js/main.js:3073-3087` hands the store a function; the function itself, at
...
announcement can be gated on the outcome: `src/js/main.js:348-350` speaks only if the write
...
surfaces, the six screens plus the blocked banner, at `src/js/main.js:5504-5524`, but the reading
...
by identity afterwards, at `src/js/main.js:2972`, which is what keeps the keyboard where the reader
left it. The row list is committed by moving nodes rather than replacing the container, at
`src/js/main.js:2860-2868`.
...
`src/js/main.js:53-54`, and the response cache is not in `localStorage` at all.
...
| `mrt.settings` | the settings form, the cover art switch, the theme control and the reading filter, at `src/js/main.js:700` | nothing | Preferences, not data. Deliberately outside the state so a settings write can never fail a progress write. |
| `sidebar.collapsed` | the sidebar toggle, at `src/js/main.js:1045` | nothing | Whether the rail is collapsed. Wrapped in its own try, because losing it is not worth an error. |
