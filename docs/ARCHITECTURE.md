...
`src/js/main.js:2093-2108`. An in-flight synopsis run is cancelled and its tab-memory prose is
...
`src/js/views/reading.js:786-788` hands the store a function; the function itself, at
...
refresh fan-out at `src/js/main.js:2525-2547`, including the rail, reading view, Home, Library hub
...
`src/js/views/reading.js:688`, which is what keeps the keyboard where the reader left it. The row list is
...
`src/js/main.js:3141-3192`. The selected id lives only in the validated `path` query of the hash
...
`src/js/main.js:3052-3084`, while the existing Store-driven render path calls the position-only
refresh at `src/js/main.js:2525-2547`. That refresh leaves cards, controls, focus, scroll and
