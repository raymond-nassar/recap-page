...
itself is loaded at `src/index.html:1133`. The launch page, which is the tab a reader's issue opens
...
`src/js/main.js:76-99`; application-wide bookkeeping lives at module scope, while constructed view
...
`src/js/main.js:2088-2103`. An in-flight synopsis run is cancelled and its tab-memory prose is
...
`src/js/main.js:1820-1830`, so browsing does not depend on the metadata service.
...
`src/js/views/reading.js:797-799` hands the store a function; the function itself, at
...
announcement can be gated on the outcome: `src/js/main.js:292-294` speaks only if the write
...
refresh fan-out at `src/js/main.js:2520-2542`, including the rail, reading view, Home, Library hub
...
`src/js/views/reading.js:699`, which is what keeps the keyboard where the reader left it. The row list is
...
sidebar preferences at `src/js/main.js:61-63`, and save education owns one more at
...
| `mrt.settings` | the settings form, cover art, theme, D shortcut, description hiding and reading filter controls, at `src/js/main.js:650-661` | nothing | Preferences, not data, and excluded from reading-progress backups and restores. Deliberately outside the state so a settings write can never fail a progress write. D shortcut and description hiding default on; failed saves apply to this tab with a visible reload warning. Description hiding stores only a boolean: actual changes reset individual disclosure choices, not tab-held prose, and never fetch. An older `cachePurge` field is read once as migration input but is no longer authoritative or written by current code. |
| `mrt.cache-purge.v1` | successful cache cleanup, at `src/js/main.js:619-637` | nothing | A monotonic cleanup generation held apart from settings so an older tab cannot lower it by serializing the settings shape it knows. Current tabs serialize its read-max-write step through one origin-wide browser lock. |
| `sidebar.collapsed` | deliberate desktop sidebar toggles, inside the persist guard at `src/js/main.js:1129-1136` | nothing | Whether the desktop rail is compact. Narrow open and closed state is ephemeral and never writes this key. Wrapped in its own try, because losing it is not worth an error. |
...
`src/js/main.js:3138-3189`. The selected id lives only in the validated `path` query of the hash
...
`src/js/main.js:118-142`; that repaint updates progress and the stop action in place at
...
`src/js/main.js:3049-3081`, while the existing Store-driven render path calls the position-only
