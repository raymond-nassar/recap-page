...
Those five are built together at `src/js/main.js:87-102`. Read that block and you have read the
...
`src/js/main.js:5310-5312`. The hydrator itself is not rebuilt; only its reference to the client is
...
`src/js/main.js:3124-3138` hands the store a function; the function itself, at
...
announcement can be gated on the outcome: `src/js/main.js:351-353` speaks only if the write
...
surfaces, the six screens plus the blocked banner, at `src/js/main.js:5637-5658`, but the reading
...
by identity afterwards, at `src/js/main.js:3023`, which is what keeps the keyboard where the reader
left it. The row list is committed by moving nodes rather than replacing the container, at
`src/js/main.js:2911-2919`.
...
| `sidebar.collapsed` | the sidebar toggle, at `src/js/main.js:1048` | nothing | Whether the rail is collapsed. Wrapped in its own try, because losing it is not worth an error. |
