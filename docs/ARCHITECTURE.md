...
`src/js/main.js:3123-3137` hands the store a function; the function itself, at
...
by identity afterwards, at `src/js/main.js:3022`, which is what keeps the keyboard where the reader
left it. The row list is committed by moving nodes rather than replacing the container, at
`src/js/main.js:2910-2918`.
