...
`src/js/main.js:95-115`; application-wide bookkeeping lives at module scope, while constructed view
...
`src/js/main.js:5472-5486`. An in-flight synopsis run is cancelled and its tab-memory prose is
...
`src/js/main.js:4441-4451`, so browsing does not depend on the metadata service.
...
`src/js/main.js:3065-3079` hands the store a function; the function itself, at
...
announcement can be gated on the outcome: `src/js/main.js:368-370` speaks only if the write
...
refresh fan-out at `src/js/main.js:5909-5930`, including the rail, reading view, Home, Library hub
...
`src/js/main.js:2960`, which is what keeps the keyboard where the reader left it. The row list is
committed by moving nodes rather than replacing the container, at
`src/js/main.js:2848-2856`.
...
`src/js/main.js:3586-3686`. The first nonempty page creates and fills its list inside one Store
update, and every later completed page uses the same boundary, at `src/js/main.js:3543-3584` and
`src/js/main.js:3698-3725`. Cancelling retires that owner before aborting its request, so a response
...
`src/js/main.js:3765-3796`. A stop before the first page creates no list, while every page already
...
sidebar preferences at `src/js/main.js:66-68`, and save education owns one more at
...
| `mrt.settings` | the settings form, the cover art switch, the theme control and the reading filter, at `src/js/main.js:742-750` | nothing | Preferences, not data. Deliberately outside the state so a settings write can never fail a progress write. An older `cachePurge` field is read once as migration input but is no longer authoritative or written by current code. |
| `mrt.cache-purge.v1` | successful cache cleanup, at `src/js/main.js:702-720` | nothing | A monotonic cleanup generation held apart from settings so an older tab cannot lower it by serializing the settings shape it knows. Current tabs serialize its read-max-write step through one origin-wide browser lock. |
| `sidebar.collapsed` | the sidebar toggle, at `src/js/main.js:1278` | nothing | Whether the rail is collapsed. Wrapped in its own try, because losing it is not worth an error. |
...
Reading paths view, at `src/js/main.js:6176-6228`. Its selector chooses one complete spine at a
...
added**, at `src/js/main.js:6329-6349`. Cross-tab state replacement and whole-origin clearing call a
progress-only repaint at `src/js/main.js:125-153`; that repaint changes only the progress outputs at
`src/js/main.js:6352-6359`, preserving the selector's DOM identity and keyboard focus.
