...
| 2 | Match between the system and the real world | 1 | Strong. Labels are written in reader language, and availability wording hedges rather than promises. `src/js/main.js:2861-2866` |
...
Confidence: Measured
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:87-92`, `src/js/main.js:4996-5018`
...
Confidence: Observed
Evidence: `src/js/main.js:4271`, `src/js/main.js:2151-2159`, `src/js/main.js:2178-2194`, against
...
Confidence: Observed
Evidence: `src/js/main.js:2178-2194`, `src/js/main.js:4646-4652`
...
blocked one
Confidence: Observed
Evidence: `src/index.html:376`, `src/js/main.js:2892-2915`
...
Backlog item: BL-026
Resolved: BL-026 added the reference to the About view at `src/index.html:855-866`, covering Enter,
...
* 2.2.2 Pause, Stop, Hide, not applicable, because the only moving element is a progress ring
  transition that already respects reduced motion. Evidence: `src/styles.css:480`,
  `src/styles.css:1376-1378`.
...
* 3.2.2 On Input passes, overturning a tool result. HTML_CodeSniffer flagged `#form-catalog-search`
  ...
  so the criterion is met. Evidence: `src/index.html:462-468`, `src/js/main.js:4171-4180`.
...
`src/styles.css:104-110`, `src/index.html:376`
...
`src/styles.css:589`. BL-166 has since renamed both to `--accent` and `--accent-text` and repainted
...
strikethrough, at `src/styles.css:705-706`. The only opacity left on a read row is on the cover
image at `src/styles.css:708`, which carries no text. Re-measured with six rows actually in the
...
`docs/ux-artifacts/pa11y-reading-seeded.json`, `src/index.html:349-384`
...
alpha at `src/styles.css:525-532`. Sampling the rendered background across all eight catalog
...
life of the product
Confidence: Measured
Evidence: `src/styles.css:777-778`, `docs/ux-artifacts/live-inspection.json`
...
`src/index.html:148`, `src/index.html:541`
...
imported
Confidence: Measured
Evidence: `docs/ux-artifacts/pa11y-landing.json`, `src/index.html:349-384`
...
Confidence: Observed
Evidence: `src/js/main.js:2801-2806`, `src/js/main.js:2861-2866`
...
`src/styles.css:645`, `src/js/main.js:2050-2053`.
...
Confidence: Observed
Evidence: `src/js/main.js:2892-2915`, `src/index.html:376`
...
`docs/ux-artifacts/live-inspection.json`, `src/styles.css:480`, `src/styles.css:1376-1378`.
...
| Reading style | Follows a long curated order end to end rather than dipping in | Strong. The product is built around order, resume and next-unread rather than around browsing. `src/index.html:349-384` | When a list is abandoned partway, what caused it? |
| Tolerance for missing metadata | High, provided the app admits what it does not know | Strong. Pending and by-hand states are surfaced rather than hidden, and this was a deliberate decision. `src/js/main.js:2805-2806` | Would you rather see a guess or a clearly marked gap? |
| Attitude to cloud services | Actively prefers local-only and treats that as the point | Strong. Recorded as a product constraint and stated in the backlog's own out-of-scope list. `PRODUCT_BACKLOG.md:436-440`, `PRODUCT_BACKLOG.md:12962` | If sync existed and was opt-in, would you turn it on? |
...
* When I finish an issue in the Marvel Unlimited reader, I want to mark it read and be shown the
  ...
  `src/index.html:349-384` and `src/index.html:376`.
* When I open a crossover I have never read, I want to know how much reading I am committing to
  ...
  `src/js/main.js:1932-1965`.
* When an issue has no metadata yet, I want the app to say so plainly, so I can tell a pending
  lookup apart from a comic that does not exist. Traced to `src/js/main.js:2805-2806`.
...
* When I follow one crossover, I want its progress counted for that list alone, so I can see how
  ...
  Hypothesis, and the gap behind existing story 4.2. Traced to `src/index.html:423-425`.
  Resolved: BL-014 scoped the count to the active list and put the choice in the view at
  `src/index.html:426-430`, with the subtitle naming whichever of the two is being counted.
...
| Checking progress | Opens Progress by series to see how far through the crossover they are | Confused. Counts include every other list they ever imported | Existing story 4.2, `src/index.html:423-425` |
