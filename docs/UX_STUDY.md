...
`src/styles.css:130-136`, `src/index.html:307-308`
...
`--red-text` for red used as text, at `src/styles.css:130-136`, and the `kbd` tint was removed at
`src/styles.css:831`. BL-166 has since renamed both to `--accent` and `--accent-text` and repainted
...
strikethrough, at `src/styles.css:975-976`. The only opacity left on a read row is on the cover
image at `src/styles.css:978`, which carries no text. Re-measured with six rows actually in the
...
alpha at `src/styles.css:752-759`. Sampling the rendered background across all eight catalog
...
life of the product
Confidence: Measured
Evidence: `src/styles.css:1049-1050`, `docs/ux-artifacts/live-inspection.json`
...
The responsive presentation is defined at `src/styles.css:1061-1078`, and the disclosure preserves
...
Confidence: Measured
Evidence: `src/styles.css:109`, `src/index.html:10`, `docs/ux-artifacts/live-inspection.json`
...
`src/styles.css:903`, `src/js/views/reading.js:283-286`.
...
Confidence: Measured
Evidence: `src/styles.css:289-295`, `src/styles.css:297-300`,
...
`docs/ux-artifacts/live-inspection.json`, `src/styles.css:704`, `src/styles.css:1774-1776`.
