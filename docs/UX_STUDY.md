...
| 2 | Match between the system and the real world | 1 | Strong. Labels are written in reader language, and availability wording hedges rather than promises. `src/js/main.js:2844-2849` |
...
| 5 | Error prevention | 1 | Strong. Unreadable saved data pauses writing rather than overwriting, and a future schema is refused. `src/index.html:158-173`, `src/js/lib/model.js:888-964` |
...
| 9 | Help users recognize, diagnose, and recover from errors | 1 | Strong where it matters most. The unreadable-data banner explains the cause, preserves the original, and offers a salvage download. `src/index.html:158-173` |
| 10 | Help and documentation | 2 | An About view and a thorough README, but no in-app reference for shortcuts or filters. `src/index.html:42-83` |
...
Confidence: Measured
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:88-93`, `src/js/main.js:5050-5069`
...
Confidence: Observed
Evidence: `src/js/main.js:4327`, `src/js/main.js:2134-2142`, `src/js/main.js:2161-2177`, against
`src/js/main.js:501-522`
...
Confidence: Observed
Evidence: `src/js/main.js:2161-2177`, `src/js/main.js:4700-4706`
...
blocked one
Confidence: Observed
Evidence: `src/index.html:377`, `src/js/main.js:2875-2898`
...
Backlog item: BL-026
Resolved: BL-026 added the reference to the About view at `src/index.html:844-855`, covering Enter,
...
* 2.2.2 Pause, Stop, Hide, not applicable, because the only moving element is a progress ring
  transition that already respects reduced motion. Evidence: `src/styles.css:486`,
  `src/styles.css:1487-1489`.
...
* 3.2.2 On Input passes, overturning a tool result. HTML_CodeSniffer flagged `#form-catalog-search`
  ...
  so the criterion is met. Evidence: `src/index.html:455-461`, `src/js/main.js:4219-4228`.
...
`src/styles.css:104-110`, `src/index.html:377`
...
`src/styles.css:595`. BL-166 has since renamed both to `--accent` and `--accent-text` and repainted
...
strikethrough, at `src/styles.css:716-717`. The only opacity left on a read row is on the cover
image at `src/styles.css:719`, which carries no text. Re-measured with six rows actually in the
...
`docs/ux-artifacts/pa11y-reading-seeded.json`, `src/index.html:350-385`
...
alpha at `src/styles.css:531-538`. Sampling the rendered background across all eight catalog
...
life of the product
Confidence: Measured
Evidence: `src/styles.css:788-789`, `docs/ux-artifacts/live-inspection.json`
...
blocking
Confidence: Observed
Evidence: `src/js/main.js:350-363`, `src/js/main.js:501-522`, `src/index.html:21`,
`src/index.html:148`, `src/index.html:546`
...
Backlog item: BL-027
Resolved: which channel a message uses is now read off the container at `src/js/main.js:356-363`
...
imported
Confidence: Measured
Evidence: `docs/ux-artifacts/pa11y-landing.json`, `src/index.html:350-385`
...
Confidence: Observed
Evidence: `src/js/main.js:2784-2789`, `src/js/main.js:2844-2849`
...
`src/styles.css:651`, `src/js/main.js:2033-2036`.
...
Confidence: Observed
Evidence: `src/js/main.js:2875-2898`, `src/index.html:377`
...
`docs/ux-artifacts/live-inspection.json`, `src/styles.css:486`, `src/styles.css:1487-1489`.

## Step 5: Information architecture and generative artifacts

### Information architecture review

Labelling is a strength. The rail groups by the reader's intent rather than by data type, using My
Library, Start from, Add issues and App, and the labels read as plain English tasks. My Library now
holds saved lists, Everything read, Progress by series and Added by hand. Evidence:
`src/index.html:42-133`.

Navigation depth is shallow, one level from the rail to any primary task, and every primary task is
findable from the rail without nesting. The five Add workflows are separate addressable screens,
while the old combined Add address resolves to Search issues for compatibility. Evidence:
`src/index.html:66-133`, `src/js/lib/route.js:14-29`.
...
src/ returning no matches`, `src/js/main.js:1210-1230`,
...
Confidence: Observed
Evidence: `src/js/main.js:119`, `src/js/lib/readingFilters.js:25-50`
Source: Step 5 information architecture review, addressability
...
| Reading style | Follows a long curated order end to end rather than dipping in | Strong. The product is built around order, resume and next-unread rather than around browsing. `src/index.html:350-385` | When a list is abandoned partway, what caused it? |
| Tolerance for missing metadata | High, provided the app admits what it does not know | Strong. Pending and by-hand states are surfaced rather than hidden, and this was a deliberate decision. `src/js/main.js:2788-2789` | Would you rather see a guess or a clearly marked gap? |
| Attitude to cloud services | Actively prefers local-only and treats that as the point | Strong. Recorded as a product constraint and stated in the backlog's own out-of-scope list. `PRODUCT_BACKLOG.md:444-448`, `PRODUCT_BACKLOG.md:13263` | If sync existed and was opt-in, would you turn it on? |
...
* When I finish an issue in the Marvel Unlimited reader, I want to mark it read and be shown the
  ...
  `src/index.html:350-385` and `src/index.html:377`.
* When I open a crossover I have never read, I want to know how much reading I am committing to
  ...
  `src/js/main.js:1915-1948`.
* When an issue has no metadata yet, I want the app to say so plainly, so I can tell a pending
  lookup apart from a comic that does not exist. Traced to `src/js/main.js:2788-2789`.
...
* When I follow one crossover, I want its progress counted for that list alone, so I can see how
  ...
  Hypothesis, and the gap behind existing story 4.2. Traced to `src/index.html:424-426`.
  Resolved: BL-014 scoped the count to the active list and put the choice in the view at
  `src/index.html:425-429`, with the subtitle naming whichever of the two is being counted.
...
| Checking progress | Opens Progress by series to see how far through the crossover they are | Confused. Counts include every other list they ever imported | Existing story 4.2, `src/index.html:424-426` |
...
**Character Spotlight scope is explicit.** The current shelf has thirteen readings across twelve
stories. Five stories appear under Best of, four under Complete guides, and four readings remain
under All only. Groot appears under All and Complete but not Best of. Twenty-four installed-Edge
assertions at 1280 by 900 and 390 by 844 confirmed those counts and no horizontal overflow. Sharing
41 source-required issues with Rocket does not combine the cards or shorten either sequence.

**Marvel on Screen scope is explicit.** The Hub adds one non-empty category after Timeline,
...
category uses the existing card grid and disappears when no matching guide is present; it is a
heading on the Hub rather than a fourth browse route. Ten installed-Edge assertions at 1280 by 900,
2048 by 1080, and 390 by 844 confirmed the four-card order, plain heading, equal-height desktop
cards, one-column narrow layout, and no horizontal overflow.
