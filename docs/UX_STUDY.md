...
| 2 | Match between the system and the real world | 1 | Strong. Labels are written in reader language, and availability wording hedges rather than promises. `src/js/main.js:3109-3114` |
...
Confidence: Measured
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:92-97`, `src/js/main.js:5442-5462`
...
Confidence: Observed
Evidence: `src/js/main.js:4703`, `src/js/main.js:2362-2370`, `src/js/main.js:2389-2405`, against
`src/js/main.js:572-593`
...
Confidence: Observed
Evidence: `src/js/main.js:2389-2405`, `src/js/main.js:5092-5098`
...
blocked one
Confidence: Observed
Evidence: `src/index.html:300`, `src/js/main.js:3140-3163`
...
Backlog item: BL-026
Resolved: BL-026 added the reference to the About view at `src/index.html:897-908`, covering Enter,
...
* 3.2.2 On Input passes, overturning a tool result. HTML_CodeSniffer flagged `#form-catalog-search`
  ...
  so the criterion is met. Evidence: `src/index.html:460-466`, `src/js/main.js:4589-4598`.
...
strikethrough, at `src/styles.css:744-745`. The only opacity left on a read row is on the cover
image at `src/styles.css:747`, which carries no text. Re-measured with six rows actually in the
...
life of the product
Confidence: Measured
Evidence: `src/styles.css:818-819`, `docs/ux-artifacts/live-inspection.json`
...
blocking
Confidence: Observed
Evidence: `src/js/main.js:417-430`, `src/js/main.js:572-593`, `src/index.html:21`,
`src/index.html:102`, `src/index.html:599`
...
Backlog item: BL-027
Resolved: which channel a message uses is now read off the container at `src/js/main.js:423-430`
...
`showView` writes it only to `.ri[data-view]` at `src/js/main.js:1383-1385`, and no rail item declares
...
Confidence: Observed
Evidence: `src/js/main.js:3049-3054`, `src/js/main.js:3109-3114`
...
Resolution evidence: the current Library hub includes both views at `src/index.html:392-425`
...
`src/styles.css:675`, `src/js/main.js:2260-2263`.
...
Confidence: Observed
Evidence: `src/js/main.js:3140-3163`, `src/index.html:300`
...
`docs/ux-artifacts/live-inspection.json`, `src/styles.css:493`, `src/styles.css:1479-1486`.
...
src/ returning no matches`, `src/js/main.js:1359-1411`,
...
Confidence: Observed
Evidence: `src/js/main.js:145`, `src/js/lib/readingFilters.js:25-50`
Source: Step 5 information architecture review, addressability
...
| Tolerance for missing metadata | High, provided the app admits what it does not know | Strong. Pending and by-hand states are surfaced rather than hidden, and this was a deliberate decision. `src/js/main.js:3053-3054` | Would you rather see a guess or a clearly marked gap? |
...
* When I open a crossover I have never read, I want to know how much reading I am committing to
  ...
  `src/js/main.js:2137-2170`.
* When an issue has no metadata yet, I want the app to say so plainly, so I can tell a pending
  lookup apart from a comic that does not exist. Traced to `src/js/main.js:3053-3054`.
...
* When I follow one crossover, I want its progress counted for that list alone, so I can see how
  ...
  Hypothesis, and the gap behind existing story 4.2. Traced to `src/index.html:429-431`.
...
| Checking progress | Opens Progress by series to see how far through the crossover they are | Confused. Counts include every other list they ever imported | Existing story 4.2, `src/index.html:429-431` |
