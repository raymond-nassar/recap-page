...
| 2 | Match between the system and the real world | 1 | Strong. Labels are written in reader language, and availability wording hedges rather than promises. `src/js/main.js:3222-3227` |
...
Confidence: Measured
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:95-100`, `src/js/main.js:5637-5658`
...
Confidence: Observed
Evidence: `src/js/main.js:4898`, `src/js/main.js:2475-2483`, `src/js/main.js:2502-2518`, against
...
Confidence: Observed
Evidence: `src/js/main.js:2502-2518`, `src/js/main.js:5287-5293`
...
blocked one
Confidence: Observed
Evidence: `src/index.html:300`, `src/js/main.js:3253-3276`
...
Backlog item: BL-026
Resolved: BL-026 added the reference to the About view at `src/index.html:899-910`, covering Enter,
...
* 2.2.2 Pause, Stop, Hide, not applicable, because the only moving element is a progress ring
  transition that already respects reduced motion. Evidence: `src/styles.css:507`,
  `src/styles.css:1507-1509`.
...
* 3.2.2 On Input passes, overturning a tool result. HTML_CodeSniffer flagged `#form-catalog-search`
  ...
  so the criterion is met. Evidence: `src/index.html:462-468`, `src/js/main.js:4784-4793`.
...
`src/styles.css:104-110`, `src/index.html:300`
...
`src/styles.css:626`. BL-166 has since renamed both to `--accent` and `--accent-text` and repainted
...
strikethrough, at `src/styles.css:767-768`. The only opacity left on a read row is on the cover
image at `src/styles.css:770`, which carries no text. Re-measured with six rows actually in the
...
`docs/ux-artifacts/pa11y-reading-seeded.json`, `src/index.html:270-308`
...
alpha at `src/styles.css:554-561`. Sampling the rendered background across all eight catalog
...
life of the product
Confidence: Measured
Evidence: `src/styles.css:841-842`, `docs/ux-artifacts/live-inspection.json`
...
blocking
Confidence: Observed
Evidence: `src/js/main.js:420-433`, `src/js/main.js:575-596`, `src/index.html:21`,
`src/index.html:102`, `src/index.html:601`
...
Backlog item: BL-027
Resolved: which channel a message uses is now read off the container at `src/js/main.js:426-433`
...
imported
Confidence: Measured
Evidence: `docs/ux-artifacts/pa11y-landing.json`, `src/index.html:270-308`
...
`showView` writes it only to `.ri[data-view]` at `src/js/main.js:1425-1427`, and no rail item declares
...
Confidence: Observed
Evidence: `src/js/main.js:3162-3167`, `src/js/main.js:3222-3227`
...
Resolution evidence: the current Library hub includes both views at `src/index.html:394-427`
...
`src/styles.css:698`, `src/js/main.js:2365-2368`.
...
Confidence: Observed
Evidence: `src/js/main.js:3253-3276`, `src/index.html:300`
...
`docs/ux-artifacts/live-inspection.json`, `src/styles.css:507`, `src/styles.css:1507-1514`.
...
Evidence: `src/index.html:42-87`, `src/js/lib/route.js:20-31`.
...
`src/js/main.js:1456-1495`, `test/ux-completion.test.js:63-75`.
...
src/ returning no matches`, `src/js/main.js:1401-1454`,
...
Confidence: Observed
Evidence: `src/js/main.js:148`, `src/js/lib/readingFilters.js:25-50`
Source: Step 5 information architecture review, addressability
...
| Reading style | Follows a long curated order end to end rather than dipping in | Strong. The product is built around order, resume and next-unread rather than around browsing. `src/index.html:270-308` | When a list is abandoned partway, what caused it? |
| Tolerance for missing metadata | High, provided the app admits what it does not know | Strong. Pending and by-hand states are surfaced rather than hidden, and this was a deliberate decision. `src/js/main.js:3166-3167` | Would you rather see a guess or a clearly marked gap? |
...
* When I finish an issue in the Marvel Unlimited reader, I want to mark it read and be shown the
  ...
  `src/index.html:270-308` and `src/index.html:300`.
* When I open a crossover I have never read, I want to know how much reading I am committing to
  ...
  `src/js/main.js:2242-2275`.
* When an issue has no metadata yet, I want the app to say so plainly, so I can tell a pending
  lookup apart from a comic that does not exist. Traced to `src/js/main.js:3166-3167`.
...
* When I follow one crossover, I want its progress counted for that list alone, so I can see how
  ...
  Hypothesis, and the gap behind existing story 4.2. Traced to `src/index.html:431-433`.
  Resolved: BL-014 scoped the count to the active list and put the choice in the view at
  `src/index.html:432-436`, with the subtitle naming whichever of the two is being counted.
...
| Checking progress | Opens Progress by series to see how far through the crossover they are | Confused. Counts include every other list they ever imported | Existing story 4.2, `src/index.html:431-433` |
