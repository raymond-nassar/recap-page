...
| 2 | Match between the system and the real world | 1 | Strong. Labels are written in reader language, and availability wording hedges rather than promises. `src/js/views/reading.js:32-37` |
...
Confidence: Measured
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:82-87`, `src/js/main.js:2462-2482`
...
Confidence: Observed
Evidence: `src/js/main.js:1826`, `src/js/views/reading.js:252-260`, `src/js/views/reading.js:277-291`, against
`src/js/main.js:499-524`
...
Confidence: Observed
Evidence: `src/js/views/reading.js:277-291`, `src/js/views/data.js:184-189`
...
blocked one
Confidence: Observed
Evidence: `src/index.html:306-307`, `src/js/views/reading.js:876-895`
...
Backlog item: BL-026
Resolved: BL-026 added the reference to the About view at `src/index.html:932-943`, covering Enter,
...
* 3.2.2 On Input passes, overturning a tool result. HTML_CodeSniffer flagged `#form-catalog-search`
  ...
  so the criterion is met. Evidence: `src/index.html:473-478`, `src/js/views/catalog.js:409-438`.
...
`src/styles.css:121-127`, `src/index.html:306-307`
...
`docs/ux-artifacts/pa11y-reading-seeded.json`, `src/index.html:276-315`
...
keyboard focus and Escape handling at `src/js/views/reading.js:780-861`.
...
blocking
Confidence: Observed
Evidence: `src/js/main.js:350-363`, `src/js/main.js:499-524`, `src/index.html:21`,
`src/index.html:108`, `src/index.html:616`
...
imported
Confidence: Measured
Evidence: `docs/ux-artifacts/pa11y-landing.json`, `src/index.html:276-315`
...
Confidence: Measured
Evidence: `src/styles.css:100`, `src/index.html:10`, `docs/ux-artifacts/live-inspection.json`
...
`showView` writes it only to `.ri[data-view]` at `src/js/main.js:1434-1436`, and no rail item declares
...
Confidence: Observed
Evidence: `src/js/views/reading.js:707-710`, `src/js/views/reading.js:32-37`
...
Resolution evidence: the current Library hub includes both views at `src/index.html:409-442`
...
`src/styles.css:894`, `src/js/views/reading.js:189-192`.
...
Confidence: Observed
Evidence: `src/js/views/reading.js:876-895`, `src/index.html:306-307`
...
Confidence: Measured
Evidence: `src/styles.css:280-286`, `src/styles.css:288-291`,
...
`docs/ux-artifacts/live-inspection.json`, `src/styles.css:695`, `src/styles.css:1765-1767`.
...
`src/js/main.js:1466-1505`, `test/ux-completion.test.js:69-80`.
...
src/ returning no matches`, `src/js/main.js:1410-1463`,
...
Confidence: Observed
Evidence: `src/js/views/reading.js:142`, `src/js/lib/readingFilters.js:25-50`
Source: Step 5 information architecture review, addressability
...
| Reading style | Follows a long curated order end to end rather than dipping in | Strong. The product is built around order, resume and next-unread rather than around browsing. `src/index.html:276-315` | When a list is abandoned partway, what caused it? |
| Tolerance for missing metadata | High, provided the app admits what it does not know | Strong. Pending and by-hand states are surfaced rather than hidden, and this was a deliberate decision. `src/js/views/reading.js:711-712` | Would you rather see a guess or a clearly marked gap? |
...
* When I finish an issue in the Marvel Unlimited reader, I want to mark it read and be shown the
  ...
  `src/index.html:276-315` and `src/index.html:306-307`.
...
* When an issue has no metadata yet, I want the app to say so plainly, so I can tell a pending
  lookup apart from a comic that does not exist. Traced to `src/js/views/reading.js:711-712`.
...
* When I follow one crossover, I want its progress counted for that list alone, so I can see how
  ...
  Hypothesis, and the gap behind existing story 4.2. Traced to `src/index.html:446-448`.
  Resolved: BL-014 scoped the count to the active list and put the choice in the view at
  `src/index.html:447-451`, with the subtitle naming whichever of the two is being counted.
...
| Checking progress | Opens Progress by series to see how far through the crossover they are | Confused. Counts include every other list they ever imported | Existing story 4.2, `src/index.html:446-448` |
