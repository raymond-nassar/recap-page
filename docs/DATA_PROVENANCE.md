...
Wraith War, Secret Wars II, and Mutant Massacre remain blocked because exact ROM, Micronauts, or
Power Pack metadata is missing; none of those source rows was omitted or replaced.
...
One hundred and fourteen files under [`src/data/`](../src/data), one per curated list, holding 4,425 issue
records covering 3,628 distinct issues. Each record copies from the upstream API: `issueId`, `title`,
`number`, `url`, `seriesId`, `seriesName`, `onSale`, `mu`, `digitalId`, `pageCount`, a `cover`
object of `path` and `ext`, and `creators` of `name` and `role`. Across the one hundred and fourteen,
4,356 records carry a cover URL and 4,272 carry creator credits.
...
| Compiled for this project from Comic Book Herald's guide | 82 | The guide's issue selection and sequence, re-expressed as local checklists. Every card links to the exact guide followed |
| Compiled for this project from Comic Book Reading Orders | 18 | Factual issue identities and order from exact event pages or visible timeline sections. Every card links to the source followed |
...
The one hundred and twelve Markdown files in [`src/data/orders/`](../src/data/orders) are authored in
this repository. Ninety-six are generated by committed scripts: eight from series metadata, seventy
from reviewed Comic Book Herald mappings, and eighteen from reviewed Comic Book Reading Orders mappings.
Sixteen are compiled by hand. Every file carries its own derivation trail. Eighty-five reference a
Comic Book Herald guide, and eighty-two name it on their catalog card and link to the exact page or
section followed. Eighteen reference Comic Book Reading Orders and name it on their card.
...
| `sourceLicense` | An SPDX expression, or `null`. Only a licence actually conveyed with the vendored order. `null` on all one hundred and fourteen shipped lists today |
...
when the selection was not made here. Eighty-two of the 102 shipped lists name Comic Book
Herald on the card and link back to the guide they follow. The other externally selected lists are
eighteen Comic Book Reading Orders guides and the two vendored checklists in the table above. Two guide
...
Eighteen lists in this repository now come from that site. Each credits Comic Book Reading Orders and
links to the exact source page and section followed, which is the pattern the reply permitted. The
site publishes no terms of use, which makes the reply the only statement of its position that exists.
...
interface in exactly one place, at `src/js/main.js:2346`, and the function behind it already
answers for the absence at `src/js/main.js:2534-2543`, with a test asserting the sentence it
