...
Two hundred and fifty-nine pinned reading-order payloads sit under
[`src/data/`](../src/data): 258 visible catalog files and one noncatalog partition parent. They hold
24,195 issue records covering 16,707 distinct issues. The visible files hold 23,708 of those records;
...
object of `path` and `ext`, and `creators` of `name` and `role`. Across all 259 payloads, 19,571
records carry a cover URL and 18,363 carry creator credits.
...
| Compiled for this project from Comic Book Herald's guide | 126 | The guide's issue selection and sequence, re-expressed as local checklists. Every visible card links to the exact guide followed |
...
The 181 source orders generate 258 visible Reading Lists. Marvel Knights to Planet X is the only
...
The 179 Markdown files in [`src/data/orders/`](../src/data/orders) are local source files.
The two upstream Hickman checklists are fetched separately when vendored. 173 local files
are generated by committed scripts: eight from series
metadata, 126 from reviewed Comic Book Herald mappings, and thirty-nine from reviewed Comic Book
Reading Orders mappings. Six local sources are compiled by hand. Every file carries its own
derivation trail. The 126 Comic Book Herald sources name that guide on their catalog cards and link
...
`src/data/curated-lists.json:5973-6011`, plus the 78-stop Marvel Knights to Planet X path generated
...
`src/data/catalog.json:9833-9958`.
