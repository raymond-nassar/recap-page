...
Two hundred and fifty-four pinned reading-order payloads sit under
[`src/data/`](../src/data): 253 visible catalog files and one noncatalog partition parent. They hold
22,570 issue records covering 15,880 distinct issues. The visible files hold 22,083 of those records;
...
object of `path` and `ext`, and `creators` of `name` and `role`. Across all 254 payloads, 18,053
records carry a cover URL and 16,940 carry creator credits.
...
Of the visible records, 999 are unopenable placeholders for unresolved source positions and 197
non-placeholder records carry neither a series ID nor a digital ID after an upstream refusal. Those
1,196 gap positions affect 24 catalog entries: nine carry placeholders, eighteen carry empty records, and
...
| Compiled for this project from Comic Book Herald's guide | 121 | The guide's issue selection and sequence, re-expressed as local checklists. Every visible card links to the exact guide followed |
...
The 176 source orders generate 253 visible Reading Lists. Marvel Knights to Planet X is the only
...
The 174 Markdown files in [`src/data/orders/`](../src/data/orders) are local source files.
The two upstream Hickman checklists are fetched separately when vendored. 168 local files
are generated by committed scripts: eight from series
metadata, 121 from reviewed Comic Book Herald mappings, and thirty-nine from reviewed Comic Book
Reading Orders mappings. Six local sources are compiled by hand. Every file carries its own
derivation trail. The 121 Comic Book Herald sources name that guide on their catalog cards and link
...
`src/data/curated-lists.json:5789-5827`, plus the 78-stop Marvel Knights to Planet X path generated
...
`src/data/catalog.json:9614-9739`.
...
The full source-library review covers 176 other orders: four partial relationships and 172
