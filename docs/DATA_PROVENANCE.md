...
Two hundred and fifty-three pinned reading-order payloads sit under
[`src/data/`](../src/data): 252 visible catalog files and one noncatalog partition parent. They hold
22,443 issue records covering 15,813 distinct issues. The visible files hold 21,956 of those records;
...
object of `path` and `ext`, and `creators` of `name` and `role`. Across all 253 payloads, 17,926
records carry a cover URL and 16,813 carry creator credits.
...
| Compiled for this project from Comic Book Herald's guide | 120 | The guide's issue selection and sequence, re-expressed as local checklists. Every visible card links to the exact guide followed |
...
The 175 source orders generate 252 visible Reading Lists. Marvel Knights to Planet X is the only
...
The 173 Markdown files in [`src/data/orders/`](../src/data/orders) include 171 local source files
and two retained upstream snapshots. 167 are generated by committed scripts: eight from series
metadata, 120 from reviewed Comic Book Herald mappings, and thirty-nine from reviewed Comic Book
Reading Orders mappings. Six local sources are compiled by hand. Every file carries its own
derivation trail. The 120 Comic Book Herald sources name that guide on their catalog cards and link
