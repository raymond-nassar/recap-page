...
| Compiled for this project from Comic Book Herald's guide | 119 | The guide's issue selection and sequence, re-expressed as local checklists. Every visible card links to the exact guide followed |
...
The 174 source orders generate 251 visible Reading Lists. Marvel Knights to Planet X is the only
...
The 172 Markdown files in [`src/data/orders/`](../src/data/orders) include 170 local source files
and two retained upstream snapshots. 166 are generated by committed scripts: eight from series
metadata, 119 from reviewed Comic Book Herald mappings, and thirty-nine from reviewed Comic Book
Reading Orders mappings. Six local sources are compiled by hand. Every file carries its own
derivation trail. The 118 Comic Book Herald sources name that guide on their catalog cards and link
...
| `sourceLicense` | An SPDX expression, or `null`. Only a licence actually conveyed with the vendored order. `null` on all 252 pinned reading-order payloads today |
...
The current Character Spotlight shelf has forty-six readings across forty-five stories. Six
readings are explicit Best of selections, thirty-four are complete guides, and six are `other`.
The two X-Men readings share one story and one classification, so the `other` group has five
...
Marvel origin and its actual route at `src/index.html:989`. Redistributing Content outside an App, and sublicensing it onward, are the
...
reaches the interface in the reading hero at `src/js/views/reading.js:465` and the issue-details view at
`src/js/views/issue.js:74`. The function behind both already answers for the absence at
`src/js/views/reading.js:77-85`, with a test asserting the sentence it returns. It is also reversible:
...
`src/data/curated-lists.json:5680-5718`, plus the 78-stop Marvel Knights to Planet X path generated
...
`src/data/catalog.json:9484-9609`.
