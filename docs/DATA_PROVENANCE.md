...
One hundred and twenty-one files under [`src/data/`](../src/data), one per curated list, holding 4,554 issue
records covering 3,733 distinct issues. Each record copies from the upstream API: `issueId`, `title`,
`number`, `url`, `seriesId`, `seriesName`, `onSale`, `mu`, `digitalId`, `pageCount`, a `cover`
object of `path` and `ext`, and `creators` of `name` and `role`. Across the one hundred and twenty-one,
4,485 records carry a cover URL and 4,399 carry creator credits.
...
| Compiled for this project from Comic Book Reading Orders | 21 | Factual issue identities and order from exact event pages or visible timeline sections. Every card links to the source followed |
...
The one hundred and nineteen Markdown files in [`src/data/orders/`](../src/data/orders) are authored in
this repository. One hundred and three are generated by committed scripts: eight from series metadata,
seventy from reviewed Comic Book Herald mappings, and twenty-five from reviewed Comic Book Reading Orders mappings.
...
section followed. Twenty-five reference Comic Book Reading Orders and name it on their card.
...
| `sourceLicense` | An SPDX expression, or `null`. Only a licence actually conveyed with the vendored order. `null` on all one hundred and twenty-one shipped lists today |
...
when the selection was not made here. Eighty-two of the 109 shipped lists name Comic Book
Herald on the card and link back to the guide they follow. The other externally selected lists are
twenty-five Comic Book Reading Orders guides and the two vendored checklists in the table above. Two guide
...
Twenty-five lists in this repository now come from that site. Each credits Comic Book Reading Orders and
...
interface in exactly one place, at `src/js/main.js:2361`, and the function behind it already
answers for the absence at `src/js/main.js:2549-2558`, with a test asserting the sentence it
