...
Two hundred and forty-eight pinned reading-order payloads sit under
[`src/data/`](../src/data): 247 visible catalog files and one noncatalog partition parent. They hold
20,650 issue records covering 15,293 distinct issues. The visible files hold 20,163 of those records;
...
object of `path` and `ext`, and `creators` of `name` and `role`. Across all 248 payloads, 16,131
records carry a cover URL and 15,176 carry creator credits.
...
| Compiled for this project from Comic Book Herald's guide | 115 | The guide's issue selection and sequence, re-expressed as local checklists. Every visible card links to the exact guide followed |
...
The 170 source orders generate 247 visible Reading Lists. Marvel Knights to Planet X is the only
...
The 170 Markdown files in [`src/data/orders/`](../src/data/orders) include 168 local source files
and two retained upstream snapshots. 162 are generated by committed scripts: eight from series
metadata, 115 from reviewed Comic Book Herald mappings, and thirty-nine from reviewed Comic Book
Reading Orders mappings. Six local sources are compiled by hand. Every file carries its own
derivation trail. The 115 Comic Book Herald sources name that guide on their catalog cards and link
...
| `sourceLicense` | An SPDX expression, or `null`. Only a licence actually conveyed with the vendored order. `null` on all 248 pinned reading-order payloads today |
