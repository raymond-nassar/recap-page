...
| Compiled for this project from Comic Book Herald's guide | 116 | The guide's issue selection and sequence, re-expressed as local checklists. Every visible card links to the exact guide followed |
...
The 171 source orders generate 248 visible Reading Lists. Marvel Knights to Planet X is the only
...
and two retained upstream snapshots. 163 are generated by committed scripts: eight from series
metadata, 116 from reviewed Comic Book Herald mappings, and thirty-nine from reviewed Comic Book
Reading Orders mappings. Six local sources are compiled by hand. Every file carries its own
derivation trail. The 116 Comic Book Herald sources name that guide on their catalog cards and link
...
reaches the interface in the reading hero at `src/js/main.js:3022` and the issue-details view at
`src/js/main.js:1834`. The function behind both already answers for the absence at
`src/js/main.js:3209-3218`, with a test asserting the sentence it returns. It is also reversible:
