...
One hundred and thirty-four files under [`src/data/`](../src/data), one per curated list, holding
4,682 issue records covering 3,852 distinct issues. Each record copies from the upstream API:
`issueId`, `title`,
...
thirty-four, 4,613 records carry a cover URL and 4,521 carry creator credits.
...
The one hundred and thirty-two Markdown files in [`src/data/orders/`](../src/data/orders) are
authored in this repository. One hundred and sixteen are generated by committed scripts: eight from
series metadata, seventy from reviewed Comic Book Herald mappings, and thirty-eight from reviewed
...
when the selection was not made here. Eighty-two of the 134 shipped lists name Comic Book Herald on
