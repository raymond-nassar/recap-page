...
Ninety-five files under [`src/data/`](../src/data), one per curated list, holding 4,092 issue records
covering 3,433 distinct issues. Each record copies from the upstream API: `issueId`, `title`,
`number`, `url`, `seriesId`, `seriesName`, `onSale`, `mu`, `digitalId`, `pageCount`, a `cover`
object of `path` and `ext`, and `creators` of `name` and `role`. Across the ninety-five, 4,023 records
carry a cover URL and 3,950 carry creator credits.
...
| Compiled for this project from Comic Book Herald's guide | 76 | The guide's issue selection and sequence, re-expressed as local checklists. Every card links to the exact guide followed |
...
The ninety-three Markdown files in [`src/data/orders/`](../src/data/orders) are authored in this
repository. Seventy-seven are generated by committed scripts: eight from series metadata, sixty-four
from reviewed Comic Book Herald mappings, and five from reviewed Comic Book Reading Orders mappings.
Sixteen are compiled by hand. Every file carries its own derivation trail. Seventy-nine reference a
Comic Book Herald guide, and seventy-six name it on their catalog card and link to the exact page or
...
The consequence is already committed. 63 of the 4,092 curated items across the ninety-five orders hold
...
when the selection was not made here. Seventy-six of the ninety-five shipped lists name Comic Book Herald
