...
| Compiled for this project from Comic Book Herald's guide | 72 | The guide's issue selection and sequence, re-expressed as local checklists. Every card links to the exact guide followed |
...
The eighty-four Markdown files in [`src/data/orders/`](../src/data/orders) are authored in this
repository. Sixty-eight are generated by committed scripts: eight from series metadata and sixty
from reviewed source mappings. Sixteen are compiled by hand. Every file carries its own derivation
trail. Seventy-five reference a Comic Book Herald guide, and seventy-two name it on their catalog card
...
when the selection was not made here. Seventy-two of the eighty-six shipped lists name Comic Book Herald
