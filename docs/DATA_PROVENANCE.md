...
| Compiled for this project from Comic Book Herald's guide | 128 | The guide's issue selection and sequence, re-expressed as local checklists. Every visible card links to the exact guide followed |
...
The 183 source orders generate 260 visible Reading Lists. Marvel Knights to Planet X is the only
...
The 181 Markdown files in [`src/data/orders/`](../src/data/orders) are local source files.
The two upstream Hickman checklists are fetched separately when vendored. 175 local files
are generated by committed scripts: eight from series
metadata, 128 from reviewed Comic Book Herald mappings, and thirty-nine from reviewed Comic Book
Reading Orders mappings. Six local sources are compiled by hand. Every file carries its own
derivation trail. The 128 Comic Book Herald sources name that guide on their catalog cards and link
...
`src/data/curated-lists.json:6009-6047`, plus the 78-stop Marvel Knights to Planet X path generated
...
`src/data/catalog.json:9876-10001`.
