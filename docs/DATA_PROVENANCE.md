...
One hundred and one files under [`src/data/`](../src/data), one per curated list, holding 4,310 issue
records covering 3,571 distinct issues. Each record copies from the upstream API: `issueId`, `title`,
`number`, `url`, `seriesId`, `seriesName`, `onSale`, `mu`, `digitalId`, `pageCount`, a `cover`
object of `path` and `ext`, and `creators` of `name` and `role`. Across the one hundred and one, 4,241
records carry a cover URL and 4,161 carry creator credits.
...
| Compiled for this project from Comic Book Reading Orders | 5 | Factual issue identities and order from five exact event pages. Every card links to the page followed |
...
The ninety-nine Markdown files in [`src/data/orders/`](../src/data/orders) are authored in this
repository. Eighty-three are generated by committed scripts: eight from series metadata, seventy
from reviewed Comic Book Herald mappings, and five from reviewed Comic Book Reading Orders mappings.
...
section followed. Five reference Comic Book Reading Orders and name it on their card.
...
| `sourceLicense` | An SPDX expression, or `null`. Only a licence actually conveyed with the vendored order. `null` on all one hundred and one shipped lists today |
...
The consequence is already committed. 63 of the 4,267 curated items across the ninety-seven orders hold
...
BL-206 added none, because every issue in them was on sale before the portal closed.
...
when the selection was not made here. Seventy-eight of the ninety-seven shipped lists name Comic Book
Herald on the card and link back to the guide they follow. The other externally selected lists are
five Comic Book Reading Orders guides and the two vendored checklists in the table above. Two guide
...
Five lists in this repository now come from that site. Each credits Comic Book Reading Orders and
links to the exact event page followed, which is the pattern the reply permitted. The site publishes
no terms of use, which makes the reply the only statement of its position that exists.
