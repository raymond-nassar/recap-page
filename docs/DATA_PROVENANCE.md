...
210 source rows. Marvel 2099 and Second Clone Saga remain deferred for separate review, and the
historical program is paused at MC2.
...
One hundred and thirty-two files under [`src/data/`](../src/data), one per curated list, holding
4,668 issue records covering 3,838 distinct issues. Each record copies from the upstream API:
`issueId`, `title`,
...
thirty-two, 4,599 records carry a cover URL and 4,507 carry creator credits.
...
| Compiled for this project from Comic Book Reading Orders | 36 | Factual issue identities and order from exact event pages or visible timeline sections. Every card links to the source followed |
...
The one hundred and thirty Markdown files in [`src/data/orders/`](../src/data/orders) are
authored in this repository. One hundred and fourteen are generated by committed scripts: eight from
series metadata, seventy from reviewed Comic Book Herald mappings, and thirty-six from reviewed
...
section followed. Thirty-six reference Comic Book Reading Orders and name it on their card.
...
when the selection was not made here. Eighty-two of the 132 shipped lists name Comic Book Herald on
the card and link back to the guide they follow. The other externally selected lists are thirty-six
...
Thirty-six lists in this repository now come from that site. Each credits Comic Book Reading Orders
