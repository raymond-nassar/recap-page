...
| Compiled for this project from Comic Book Herald's guide | 73 | The guide's issue selection and sequence, re-expressed as local checklists. Every card links to the exact guide followed |
...
The eighty-five Markdown files in [`src/data/orders/`](../src/data/orders) are authored in this
repository. Sixty-nine are generated by committed scripts: eight from series metadata and sixty-one
from reviewed source mappings. Sixteen are compiled by hand. Every file carries its own derivation
trail. Seventy-six reference a Comic Book Herald guide, and seventy-three name it on their catalog
...
| `sourceLicense` | An SPDX expression, or `null`. Only a licence actually conveyed with the vendored order. `null` on all eighty-seven shipped lists today |
...
when the selection was not made here. Seventy-three of the eighty-seven shipped lists name Comic Book Herald
...
Marvel origin and its actual route at `src/index.html:285`. Redistributing Content outside an App, and sublicensing it onward, are the
...
interface in exactly one place, at `src/js/main.js:2462`, and the function behind it already
answers for the absence at `src/js/main.js:2650-2659`, with a test asserting the sentence it
