...
Two hundred and fifty-five pinned reading-order payloads sit under
[`src/data/`](../src/data): 254 visible catalog files and one noncatalog partition parent. They hold
23,054 issue records covering 16,117 distinct issues. The visible files hold 22,567 of those records;
...
object of `path` and `ext`, and `creators` of `name` and `role`. Across all 255 payloads, 18,491
records carry a cover URL and 17,332 carry creator credits.
...
Of the visible records, 1,003 are unopenable placeholders for unresolved source positions and 197
non-placeholder records carry neither a series ID nor a digital ID after an upstream refusal. Those
1,200 gap positions affect 25 catalog entries: ten carry placeholders, eighteen carry empty records, and
...
| Compiled for this project from Comic Book Herald's guide | 122 | The guide's issue selection and sequence, re-expressed as local checklists. Every visible card links to the exact guide followed |
...
The 177 source orders generate 254 visible Reading Lists. Marvel Knights to Planet X is the only
...
The 175 Markdown files in [`src/data/orders/`](../src/data/orders) are local source files.
The two upstream Hickman checklists are fetched separately when vendored. 169 local files
are generated by committed scripts: eight from series
metadata, 122 from reviewed Comic Book Herald mappings, and thirty-nine from reviewed Comic Book
Reading Orders mappings. Six local sources are compiled by hand. Every file carries its own
derivation trail. The 122 Comic Book Herald sources name that guide on their catalog cards and link
...
| `sourceLicense` | An SPDX expression, or `null`. Only a licence actually conveyed with the vendored order. `null` on all 253 pinned reading-order payloads today |
...
`src/data/curated-lists.json:5792-5830`, plus the 78-stop Marvel Knights to Planet X path generated
...
`src/data/catalog.json:9617-9742`.
...
reuse is not a fresh HTTP response. The complete-library review covers 176 other source orders:
30 partial relationships, four existing-subset relationships and 142 with no shared issue.
