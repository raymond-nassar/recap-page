...
The one hundred and fifteen Markdown files in [`src/data/orders/`](../src/data/orders) are authored in
this repository. Ninety-nine are generated by committed scripts: eight from series metadata, seventy
from reviewed Comic Book Herald mappings, and twenty-one from reviewed Comic Book Reading Orders mappings.
...
section followed. Twenty-one reference Comic Book Reading Orders and name it on their card.
...
when the selection was not made here. Eighty-two of the 105 shipped lists name Comic Book
Herald on the card and link back to the guide they follow. The other externally selected lists are
twenty-one Comic Book Reading Orders guides and the two vendored checklists in the table above. Two guide
...
Twenty-one lists in this repository now come from that site. Each credits Comic Book Reading Orders and
