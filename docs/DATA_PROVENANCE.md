...
One hundred and twenty-five files under [`src/data/`](../src/data), one per curated list, holding
4,600 issue records covering 3,779 distinct issues. Each record copies from the upstream API:
`issueId`, `title`,
...
twenty-five, 4,531 records carry a cover URL and 4,439 carry creator credits.
...
| Compiled for this project from Comic Book Reading Orders | 29 | Factual issue identities and order from exact event pages or visible timeline sections. Every card links to the source followed |
...
The one hundred and twenty-three Markdown files in [`src/data/orders/`](../src/data/orders) are
authored in this repository. One hundred and seven are generated by committed scripts: eight from
series metadata, seventy from reviewed Comic Book Herald mappings, and twenty-nine from reviewed
...
section followed. Twenty-nine reference Comic Book Reading Orders and name it on their card.
...
when the selection was not made here. Eighty-two of the 113 shipped lists name Comic Book Herald on
the card and link back to the guide they follow. The other externally selected lists are twenty-nine
