...
Two hundred and sixty pinned reading-order payloads sit under
[`src/data/`](../src/data): 259 visible catalog files and one noncatalog partition parent. They hold
24,282 issue records covering 16,718 distinct tracked identities. The visible files hold 23,795 of those records;
...
object of `path` and `ext`, and `creators` of `name` and `role`. Across all 260 payloads, 19,661
records carry a cover URL and 18,453 carry creator credits.
...
Of the visible records, 1,018 are unopenable placeholders for unresolved source positions and 197
non-placeholder records carry neither a series ID nor a digital ID after an upstream refusal. Those
1,215 gap positions affect 26 catalog entries: eleven carry placeholders, eighteen carry empty records, and
...
| Compiled for this project from Comic Book Herald's guide | 127 | The guide's issue selection and sequence, re-expressed as local checklists. Every visible card links to the exact guide followed |
...
The 182 source orders generate 259 visible Reading Lists. Marvel Knights to Planet X is the only
...
The 180 Markdown files in [`src/data/orders/`](../src/data/orders) are local source files.
The two upstream Hickman checklists are fetched separately when vendored. 174 local files
are generated by committed scripts: eight from series
metadata, 127 from reviewed Comic Book Herald mappings, and thirty-nine from reviewed Comic Book
Reading Orders mappings. Six local sources are compiled by hand. Every file carries its own
derivation trail. The 127 Comic Book Herald sources name that guide on their catalog cards and link
