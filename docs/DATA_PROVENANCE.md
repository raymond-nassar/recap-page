...
Ninety-four files under [`src/data/`](../src/data), one per curated list, holding 4,017 issue records
covering 3,375 distinct issues. Each record copies from the upstream API: `issueId`, `title`,
`number`, `url`, `seriesId`, `seriesName`, `onSale`, `mu`, `digitalId`, `pageCount`, a `cover`
object of `path` and `ext`, and `creators` of `name` and `role`. Across the ninety-four, 3,948 records
carry a cover URL and 3,879 carry creator credits.
...
| Compiled for this project from Comic Book Herald's guide | 75 | The guide's issue selection and sequence, re-expressed as local checklists. Every card links to the exact guide followed |
...
The ninety-two Markdown files in [`src/data/orders/`](../src/data/orders) are authored in this
repository. Seventy-six are generated by committed scripts: eight from series metadata, sixty-three
from reviewed Comic Book Herald mappings, and five from reviewed Comic Book Reading Orders mappings.
Sixteen are compiled by hand. Every file carries its own derivation trail. Seventy-eight reference a
Comic Book Herald guide, and seventy-five name it on their catalog card and link to the exact page or
...
The consequence is already committed. 63 of the 4,017 curated items across the ninety-four orders hold
...
BL-205 added none, because every issue in them was on sale before the portal closed.
...
Nothing already saved is affected. The tree holds 3,335 distinct cover URLs across 3,948 records; 60
...
when the selection was not made here. Seventy-five of the ninety-four shipped lists name Comic Book Herald
