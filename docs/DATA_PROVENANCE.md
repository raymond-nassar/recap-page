...
Two hundred and fifty-seven pinned reading-order payloads sit under
[`src/data/`](../src/data): 256 visible catalog files and one noncatalog partition parent. They hold
23,531 issue records covering 16,336 distinct issues. The visible files hold 23,044 of those records;
...
object of `path` and `ext`, and `creators` of `name` and `role`. Across all 257 payloads, 18,947
records carry a cover URL and 17,779 carry creator credits.
...
Of the visible records, 1,004 are unopenable placeholders for unresolved source positions and 199
non-placeholder records carry neither a series ID nor a digital ID after an upstream refusal. Those
1,203 gap positions affect 26 catalog entries: eleven carry placeholders, nineteen carry empty records, and
...
| Compiled for this project from Comic Book Herald's guide | 124 | The guide's issue selection and sequence, re-expressed as local checklists. Every visible card links to the exact guide followed |
...
The 179 source orders generate 256 visible Reading Lists. Marvel Knights to Planet X is the only
...
The 177 Markdown files in [`src/data/orders/`](../src/data/orders) are local source files.
The two upstream Hickman checklists are fetched separately when vendored. 171 local files
are generated by committed scripts: eight from series
metadata, 124 from reviewed Comic Book Herald mappings, and thirty-nine from reviewed Comic Book
Reading Orders mappings. Six local sources are compiled by hand. Every file carries its own
derivation trail. The 124 Comic Book Herald sources name that guide on their catalog cards and link
...
| `sourceLicense` | An SPDX expression, or `null`. Only a licence actually conveyed with the vendored order. `null` on all 256 pinned reading-order payloads today |
...
reaches the interface in the reading hero at `src/js/views/reading.js:569` and the issue-details view at
...
`src/data/curated-lists.json:5863-5901`, plus the 78-stop Marvel Knights to Planet X path generated
...
`src/data/catalog.json:9702-9827`.
...
reuse is not a fresh HTTP response. The complete-library review covers 178 other source orders:
31 partial relationships, four existing-subset relationships and 143 with no shared issue.
