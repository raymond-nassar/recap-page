...
Eighty-nine curated-list files under [`src/data/`](../src/data), one per curated list, hold 3,994 issue
records covering 3,352 distinct issues. Each record copies from the upstream API: `issueId`, `title`,
`number`, `url`, `seriesId`, `seriesName`, `onSale`, `mu`, `digitalId`, `pageCount`, a `cover`
object of `path` and `ext`, and `creators` of `name` and `role`. Across the eighty-nine, 3,925 records
carry a cover URL and 3,856 carry creator credits.
...
The eighty-seven Markdown files in [`src/data/orders/`](../src/data/orders) are authored in this
repository. Eighty-three are generated by committed scripts: eight from series metadata and seventy-five
from reviewed source mappings. Four are compiled by hand. Every file carries its own derivation
trail. Seventy-eight reference a Comic Book Herald guide, and seventy-five name it on their catalog
card and link to the exact page or section followed.
...
The consequence is already committed. 63 of the 3,994 curated items across the eighty-nine Reading Lists hold
...
interface in exactly one place, at `src/js/main.js:2342`, and the function behind it already
answers for the absence at `src/js/main.js:2530-2539`, with a test asserting the sentence it
