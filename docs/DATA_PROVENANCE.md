...
| Compiled for this project from Comic Book Reading Orders | 10 | Factual issue identities and order from exact event pages or visible timeline sections. Every card links to the source followed |
...
The one hundred and four Markdown files in [`src/data/orders/`](../src/data/orders) are authored in
this repository. Eighty-eight are generated by committed scripts: eight from series metadata, seventy
from reviewed Comic Book Herald mappings, and ten from reviewed Comic Book Reading Orders mappings.
...
section followed. Ten reference Comic Book Reading Orders and name it on their card.
...
Ten lists in this repository now come from that site. Each credits Comic Book Reading Orders and
