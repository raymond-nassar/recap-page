...
Marvel origin and its actual route at `src/index.html:991`. Redistributing Content outside an App, and sublicensing it onward, are the
...
reaches the interface in the reading hero at `src/js/views/reading.js:474` and the issue-details view at
`src/js/views/issue.js:87`. The function behind both already answers for the absence at
`src/js/views/reading.js:78-86`, with a test asserting the sentence it returns. It is also reversible:
