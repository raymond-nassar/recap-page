...
Two hundred and fifty-eight pinned reading-order payloads sit under
[`src/data/`](../src/data): 257 visible catalog files and one noncatalog partition parent. They hold
23,701 issue records covering 16,449 distinct issues. The visible files hold 23,214 of those records;
...
object of `path` and `ext`, and `creators` of `name` and `role`. Across all 258 payloads, 19,102
records carry a cover URL and 17,918 carry creator credits.
...
Of the visible records, 1,019 are unopenable placeholders for unresolved source positions and 199
non-placeholder records carry neither a series ID nor a digital ID after an upstream refusal. Those
1,218 gap positions affect 27 catalog entries: twelve carry placeholders, nineteen carry empty records, and
...
| Compiled for this project from Comic Book Herald's guide | 125 | The guide's issue selection and sequence, re-expressed as local checklists. Every visible card links to the exact guide followed |
...
The 180 source orders generate 257 visible Reading Lists. Marvel Knights to Planet X is the only
...
The 178 Markdown files in [`src/data/orders/`](../src/data/orders) are local source files.
The two upstream Hickman checklists are fetched separately when vendored. 172 local files
are generated by committed scripts: eight from series
metadata, 125 from reviewed Comic Book Herald mappings, and thirty-nine from reviewed Comic Book
Reading Orders mappings. Six local sources are compiled by hand. Every file carries its own
derivation trail. The 125 Comic Book Herald sources name that guide on their catalog cards and link
...
| `sourceLicense` | An SPDX expression, or `null`. Only a licence actually conveyed with the vendored order. `null` on all 257 pinned reading-order payloads today |
...
Marvel origin and its actual route at `src/index.html:1055`. Redistributing Content outside an App, and sublicensing it onward, are the
...
reaches the interface in the reading hero at `src/js/views/reading.js:579` and the issue-details view at
