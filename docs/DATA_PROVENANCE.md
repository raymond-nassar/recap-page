...
Two hundred and thirty-eight pinned reading-order payloads sit under
[`src/data/`](../src/data): 237 visible catalog files and one noncatalog partition parent. They hold
16,940 issue records covering 12,839 distinct issues. The visible files hold 16,453 of those records;
...
object of `path` and `ext`, and `creators` of `name` and `role`. Across all 238 payloads, 13,818
records carry a cover URL and 12,979 carry creator credits.
...
Of the visible records, 1,386 carry neither a series ID nor a digital ID. 1,308 are explicit
placeholders for unresolved source positions and 78 record an upstream refusal. One additional
placeholder carries partial metadata, bringing the visible placeholder total to 1,309. Placeholder
...
| Compiled for this project from Comic Book Herald's guide | 106 | The guide's issue selection and sequence, re-expressed as local checklists. Every visible card links to the exact guide followed |
...
The 160 source orders generate 237 visible Reading Lists. Marvel Knights to Planet X is the only
...
The 160 Markdown files in [`src/data/orders/`](../src/data/orders) include 158 local source files
and two retained upstream snapshots. 152 are generated by committed scripts: eight from series
metadata, 106 from reviewed Comic Book Herald mappings, and thirty-eight from reviewed Comic Book
Reading Orders mappings. Six local sources are compiled by hand. Every file carries its own
derivation trail. The 106 Comic Book Herald sources name that guide on their catalog cards and link
...
| `sourceLicense` | An SPDX expression, or `null`. Only a licence actually conveyed with the vendored order. `null` on all 238 pinned reading-order payloads today |
...
Marvel origin and its actual route at `src/index.html:965`. Redistributing Content outside an App, and sublicensing it onward, are the
...
reaches the interface in the reading hero at `src/js/main.js:2803` and the issue-details view at
`src/js/main.js:1615`. The function behind both already answers for the absence at
`src/js/main.js:2990-2999`, with a test asserting the sentence it returns. It is also reversible:
