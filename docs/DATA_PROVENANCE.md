...
Two hundred and fifty-six pinned reading-order payloads sit under
[`src/data/`](../src/data): 255 visible catalog files and one noncatalog partition parent. They hold
23,126 issue records covering 16,204 distinct issues. The visible files hold 22,639 of those records;
...
object of `path` and `ext`, and `creators` of `name` and `role`. Across all 256 payloads, 18,591
records carry a cover URL and 17,460 carry creator credits.
...
Of the visible records, 1,015 are unopenable placeholders for unresolved source positions and 199
non-placeholder records carry neither a series ID nor a digital ID after an upstream refusal. Those
1,214 gap positions affect 26 catalog entries: eleven carry placeholders, nineteen carry empty records, and
...
| Compiled for this project from Comic Book Herald's guide | 123 | The guide's issue selection and sequence, re-expressed as local checklists. Every visible card links to the exact guide followed |
...
The 178 source orders generate 255 visible Reading Lists. Marvel Knights to Planet X is the only
...
The 176 Markdown files in [`src/data/orders/`](../src/data/orders) are local source files.
The two upstream Hickman checklists are fetched separately when vendored. 170 local files
are generated by committed scripts: eight from series
metadata, 123 from reviewed Comic Book Herald mappings, and thirty-nine from reviewed Comic Book
Reading Orders mappings. Six local sources are compiled by hand. Every file carries its own
derivation trail. The 123 Comic Book Herald sources name that guide on their catalog cards and link
...
`src/data/curated-lists.json:5824-5862`, plus the 78-stop Marvel Knights to Planet X path generated
...
`src/data/catalog.json:9656-9781`.
...
The full source-library review covers 177 other orders: five partial relationships and 172
