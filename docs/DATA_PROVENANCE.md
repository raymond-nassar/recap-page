...
Ninety-seven files under [`src/data/`](../src/data), one per curated list, holding 4,267 issue records
covering 3,537 distinct issues. Each record copies from the upstream API: `issueId`, `title`,
`number`, `url`, `seriesId`, `seriesName`, `onSale`, `mu`, `digitalId`, `pageCount`, a `cover`
object of `path` and `ext`, and `creators` of `name` and `role`. Across the ninety-seven, 4,198 records
carry a cover URL and 4,121 carry creator credits.
...
| Compiled for this project from Comic Book Herald's guide | 77 | The guide's issue selection and sequence, re-expressed as local checklists. Every card links to the exact guide followed |
...
The ninety-five Markdown files in [`src/data/orders/`](../src/data/orders) are authored in this
repository. Seventy-nine are generated by committed scripts: eight from series metadata, sixty-six
from reviewed Comic Book Herald mappings, and five from reviewed Comic Book Reading Orders mappings.
Sixteen are compiled by hand. Every file carries its own derivation trail. Eighty-one reference a
Comic Book Herald guide, and seventy-eight name it on their catalog card and link to the exact page or
...
| `sourceLicense` | An SPDX expression, or `null`. Only a licence actually conveyed with the vendored order. `null` on all ninety-seven shipped lists today |
...
interface in exactly one place, at `src/js/main.js:2343`, and the function behind it already
answers for the absence at `src/js/main.js:2531-2540`, with a test asserting the sentence it
