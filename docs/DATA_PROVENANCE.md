...
One hundred and six files under [`src/data/`](../src/data), one per curated list, holding 4,342 issue
records covering 3,594 distinct issues. Each record copies from the upstream API: `issueId`, `title`,
`number`, `url`, `seriesId`, `seriesName`, `onSale`, `mu`, `digitalId`, `pageCount`, a `cover`
object of `path` and `ext`, and `creators` of `name` and `role`. Across the one hundred and six, 4,273
records carry a cover URL and 4,193 carry creator credits.
...
| Compiled for this project from Comic Book Reading Orders | 15 | Factual issue identities and order from exact event pages or visible timeline sections. Every card links to the source followed |
...
The one hundred and nine Markdown files in [`src/data/orders/`](../src/data/orders) are authored in
this repository. Ninety-three are generated by committed scripts: eight from series metadata, seventy
from reviewed Comic Book Herald mappings, and fifteen from reviewed Comic Book Reading Orders mappings.
...
section followed. Fifteen reference Comic Book Reading Orders and name it on their card.
...
| `sourceLicense` | An SPDX expression, or `null`. Only a licence actually conveyed with the vendored order. `null` on all one hundred and six shipped lists today |
...
when the selection was not made here. Seventy-eight of the 102 shipped lists name Comic Book
Herald on the card and link back to the guide they follow. The other externally selected lists are
ten Comic Book Reading Orders guides and the two vendored checklists in the table above. Two guide
...
Fifteen lists in this repository now come from that site. Each credits Comic Book Reading Orders and
