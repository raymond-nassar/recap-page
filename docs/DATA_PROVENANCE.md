...
| Compiled for this project from Comic Book Herald's guide | 118 | The guide's issue selection and sequence, re-expressed as local checklists. Every visible card links to the exact guide followed |
...
The 173 source orders generate 250 visible Reading Lists. Marvel Knights to Planet X is the only
...
The 171 Markdown files in [`src/data/orders/`](../src/data/orders) include 169 local source files
and two retained upstream snapshots. 165 are generated by committed scripts: eight from series
metadata, 118 from reviewed Comic Book Herald mappings, and thirty-nine from reviewed Comic Book
...
The current Character Spotlight shelf has forty-five readings across forty-four stories. Six
readings are explicit Best of selections, thirty-three are complete guides, and six are `other`.
...
reaches the interface in the reading hero at `src/js/main.js:3059` and the issue-details view at
`src/js/main.js:1842`. The function behind both already answers for the absence at
`src/js/main.js:3246-3255`, with a test asserting the sentence it returns. It is also reversible:
...
`src/data/curated-lists.json:5649-5687`, plus the 78-stop Marvel Knights to Planet X path generated
...
`src/data/catalog.json:9446-9571`.
