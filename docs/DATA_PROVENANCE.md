...
Ninety-six files under [`src/data/`](../src/data), one per curated list, holding 4,168 issue records
covering 3,458 distinct issues. Each record copies from the upstream API: `issueId`, `title`,
`number`, `url`, `seriesId`, `seriesName`, `onSale`, `mu`, `digitalId`, `pageCount`, a `cover`
object of `path` and `ext`, and `creators` of `name` and `role`. Across the ninety-six, 4,099 records
carry a cover URL and 4,022 carry creator credits.
...
The ninety-four Markdown files in [`src/data/orders/`](../src/data/orders) are authored in this
repository. Seventy-eight are generated by committed scripts: eight from series metadata, sixty-five
from reviewed Comic Book Herald mappings, and five from reviewed Comic Book Reading Orders mappings.
Sixteen are compiled by hand. Every file carries its own derivation trail. Eighty reference a
Comic Book Herald guide, and seventy-seven name it on their catalog card and link to the exact page or
...
| `sourceLicense` | An SPDX expression, or `null`. Only a licence actually conveyed with the vendored order. `null` on all ninety-six shipped lists today |
...
when the selection was not made here. Seventy-seven of the ninety-six shipped lists name Comic Book Herald
