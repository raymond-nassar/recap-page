...
| Compiled for this project from Comic Book Herald's guide | 83 | The guide's issue selection and sequence, re-expressed as local checklists. Every card links to the exact guide followed |
...
The one hundred and thirty-three Markdown files in [`src/data/orders/`](../src/data/orders) are
authored in this repository. One hundred and seventeen are generated by committed scripts: eight from
series metadata, seventy-one from reviewed Comic Book Herald mappings, and thirty-eight from reviewed
Comic Book Reading Orders mappings.
Sixteen are compiled by hand. Every file carries its own derivation trail. Eighty-six reference a
Comic Book Herald guide, and eighty-three name it on their catalog card and link to the exact page or
...
when the selection was not made here. Eighty-three of the 135 shipped lists name Comic Book Herald on
