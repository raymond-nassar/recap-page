...
22,534 issue records covering 15,821 distinct issues. The visible files hold 22,047 of those records;
...
object of `path` and `ext`, and `creators` of `name` and `role`. Across all 254 payloads, 17,999
records carry a cover URL and 16,879 carry creator credits.
...
The 174 Markdown files in [`src/data/orders/`](../src/data/orders) include 172 local source files
and two retained upstream snapshots. 168 are generated by committed scripts: eight from series
...
| `sourceLicense` | An SPDX expression, or `null`. Only a licence actually conveyed with the vendored order. `null` on all 254 pinned reading-order payloads today |
...
`src/data/curated-lists.json:5752-5790`, plus the 78-stop Marvel Knights to Planet X path generated
...
`src/data/catalog.json:9570-9695`.
