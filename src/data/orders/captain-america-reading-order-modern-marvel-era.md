...
Source guide from Comic Book Herald, preserved as explicit provenance.

Derived from the rendered Comic Book Herald page captured for this issue, then normalized to the repository's checklist format. See scripts/vendor-orders.mjs for the published data shape and docs/DATA_PROVENANCE.md for the publication boundary this checklist stays within.

Each quoted row preserves one of the 69 frozen source positions in source order. A quoted row without a child issue is a source-semantic exclusion because it names a collection, trade, range, compound crossover row, event or order label, or direct series context rather than one atomic comic. The two Captain America: Steve Rogers Vol. 3 rows remain separate because the source places them in different reading contexts.
...
- [ ] [Captain America (2004) #25 <!-- mrt:source-occurrence=18 -->](https://www.marvel.com/comics/issue/6010/captain_america_2004_25)
